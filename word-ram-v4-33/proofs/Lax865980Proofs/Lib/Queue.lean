...
def layout : Lax865980Proofs.Compile.Layout := ⟨["hd", "tl", "x", "y", "r"], ["q"], 3⟩

/-- The machine program. -/
def prog : Lax865980.Ram.Program :=
  Lax865980Proofs.Compile.compileProgram layout (demoWatched "q" "hd" "tl" "x" "y" "r")
...
theorem demoWatched_ok :
    Lax865980Proofs.Compile.Com.Ok layout (demoWatched "q" "hd" "tl" "x" "y" "r") := by
  simp [demoWatched, push, front, advance, drain, layout, Lax865980Proofs.Compile.Com.Ok,
    Lax865980Proofs.Compile.Cond.Ok, Lax865980Proofs.Compile.condExpr, Lax865980Proofs.Compile.Expr.Ok]
...
def demoRun : Option (List ℕ × ℕ) := runOut 16 2000 prog (Lax865980.Ram.initState []) 0
