...
0. **One namespace per module inside `Lib`.** Everything a module owns
   ...
   exist and a downstream `open Lax865980Proofs.Reasoning.Lib` stays
...
def layout : Lax865980Proofs.Compile.Layout := ⟨["i", "r"], ["mk"], 2⟩

/-- The machine program. -/
def prog : Lax865980.Ram.Program :=
  Lax865980Proofs.Compile.compileProgram layout (demoWatched "mk" "i" "r")

/-- The layout covers the block, so the compilation is the one the
simulation theorem is about and not an accident. -/
theorem demoWatched_ok : Lax865980Proofs.Compile.Com.Ok layout (demoWatched "mk" "i" "r") := by
  simp [demoWatched, mark, unmark, test, layout, Lax865980Proofs.Compile.Com.Ok,
...
def demoRun : Option (List ℕ × ℕ) := runOut 16 1000 prog (Lax865980.Ram.initState []) 0
