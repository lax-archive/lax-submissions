...
named under `Lax865980Proofs` in a consumer package. -/

@[simp] theorem runOut_zero (w : ℕ) (p : Lax865980.Ram.Program) (s : Lax865980.Ram.State) (k : ℕ) :
    runOut w 0 p s k = none := by simp [runOut]

theorem runOut_succ (w fuel : ℕ) (p : Lax865980.Ram.Program) (s : Lax865980.Ram.State) (k : ℕ) :
    runOut w (fuel + 1) p s k =
      match Lax865980.Ram.step w p s with
      | none => some (s.out, k)
