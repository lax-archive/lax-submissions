...
def layout : Lax865980Proofs.Compile.Layout := ⟨["i", "m"], ["ar"], 2⟩

/-- The machine program. -/
def prog : Lax865980.Ram.Program :=
  Lax865980Proofs.Compile.compileProgram layout (demoWatched "ar" "i" "m")

/-- The layout covers the block, so the compilation is the one the
simulation theorem is about and not an accident. -/
theorem demoWatched_ok : Lax865980Proofs.Compile.Com.Ok layout (demoWatched "ar" "i" "m") := by
  simp [demoWatched, demo, put, layout, Lax865980Proofs.Compile.Com.Ok,
