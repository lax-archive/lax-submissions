...
`Lax271696.InstanceEncoding` presents a compressed sparse row block followed by an
...
open Lax271696Proofs.CliqueExpr Lax271696Proofs.TreeFold Lax271696Proofs.MsoTable
open Lax271696Proofs.CC (readLoop)
open Lax271696.InstanceEncoding (nodeCount parent opCode vertexName EncodesExprTree EncodesExpr
...
`Lax271696.InstanceEncoding.EncodesExpr` and `MsoTable.EncExpr` are the same
...
theorem children_eq (par : ℕ → ℕ) (i : ℕ) :
    Lax271696.InstanceEncoding.children par i = TreeFold.children par i := rfl
