...
surface**, in `concepts/Lax271696/CliqueExpr.lean`, and is re-exported here
...
namespace Lax271696Proofs.CliqueExpr
...
`Lax271696.CliqueExpr`; the constructors are re-exported too, so that every
use site below reads as if they were declared here. The decidable
adjacency instance comes along with `graph` as an instance. -/

export Lax271696.CliqueExpr (Expr Expr.leaf Expr.union Expr.addEdges Expr.relabel
...
export Lax271696.CliqueExpr (Op Op.union Op.leaf Op.eta Op.rho Op.code opCard)
...
end Lax271696Proofs.CliqueExpr
