...
the constant `841` absorbs it into the linear bound.
...
namespace Lax271696Proofs.CCMain

open Lax808846.Ram Lax808846.RamComputes Lax271696.GraphEncoding Lax271696.ConnectedComponents
open Lax808846Proofs.Imp Lax808846Proofs.Compile Lax808846Proofs.Reasoning Lax808846Proofs.Transfer
open Lax271696Proofs.CC
...
theorem ccCom_solves (n : ℕ) (G : SimpleGraph (Fin n)) (w : ℕ) :
    Solves layout ccCom {x | EncodesGraph x n G ∧ 841 * (x.length + 1) ≤ 2 ^ w}
...
/--
---
conclusion: Lax271696.ConnectedComponents.exists_linearTime_program_ccLabels
---
...
machine steps per unit of IMP+ cost, plus one for the final `halt`.
...
theorem exists_linearTime_program_ccLabels :
  ...
  refine ⟨ccProgram, 841, fun n G w =>
