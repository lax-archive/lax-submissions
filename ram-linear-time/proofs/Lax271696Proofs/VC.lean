...
open Lax808846.Ram Lax808846.RamComputes Lax271696.GraphEncoding
open Lax808846Proofs.Imp Lax808846Proofs.Compile Lax808846Proofs.Reasoning Lax271696Proofs.CC
...
#guard test ([3, 3, 0, 2, 4, 6, 1, 2, 0, 2, 0, 1] ++ [0]) = some ([0], 485)
#guard test ([3, 3, 0, 2, 4, 6, 1, 2, 0, 2, 0, 1] ++ [1]) = some ([0], 1338)
#guard test ([3, 3, 0, 2, 4, 6, 1, 2, 0, 2, 0, 1] ++ [2]) = some ([1], 1431)
-- the star on three leaves: its center is a cover
#guard test ([4, 3, 0, 3, 4, 5, 6, 1, 2, 3, 0, 0, 0] ++ [1]) = some ([1], 1084)
-- the path on four vertices: cover number two
#guard test ([4, 3, 0, 1, 3, 5, 6, 1, 0, 2, 1, 3, 2] ++ [1]) = some ([0], 1565)
#guard test ([4, 3, 0, 1, 3, 5, 6, 1, 0, 2, 1, 3, 2] ++ [2]) = some ([1], 2022)
-- no edges: the empty set is a cover, even with no budget
#guard test ([2, 0, 0, 0, 0] ++ [0]) = some ([1], 179)
-- no vertices
#guard test ([0, 0, 0] ++ [0]) = some ([1], 129)
-- a malformed word: the program merely halts (the exhausted tape stops it)
#guard test [5, 2, 0, 1, 9, 3] = some ([], 120)
