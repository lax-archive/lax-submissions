...
`concepts/Lax271696/Mso.lean`, and are re-exported here under the names the
...
namespace Lax271696Proofs.MsoTypes

/-! ### Syntax

The syntax, the quantifier rank and satisfaction are the surface
definitions of `Lax271696.Mso`; the constructors are re-exported too, so
that every use site below reads as if they were declared here. -/

export Lax271696.Mso (MSO MSO.adj MSO.eq MSO.mem MSO.not MSO.and MSO.exV MSO.exS rank Sat)
