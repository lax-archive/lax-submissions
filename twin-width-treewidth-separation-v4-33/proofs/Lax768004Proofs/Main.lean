...
namespace Lax768004Proofs.Main
...
theorem homogeneous_iff {A B : Finset V} :
    Lax768004.TwinWidth.Homogeneous G A B ↔
...
theorem homogeneous_comm {A B : Finset V} :
    Lax768004.TwinWidth.Homogeneous G A B ↔ Lax768004.TwinWidth.Homogeneous G B A := by
...
theorem homogeneous_union_left [DecidableEq V] {A B Y : Finset V}
    ...
    Lax768004.TwinWidth.Homogeneous G (A ∪ B) Y ↔
      (Lax768004.TwinWidth.Homogeneous G A Y ∧ Lax768004.TwinWidth.Homogeneous G B Y ∧
        (CompleteBetween G A Y ↔ CompleteBetween G B Y)) := by
...
theorem homogeneous_union_right [DecidableEq V] {A B X : Finset V}
    ...
    Lax768004.TwinWidth.Homogeneous G X (A ∪ B) ↔
      (Lax768004.TwinWidth.Homogeneous G X A ∧ Lax768004.TwinWidth.Homogeneous G X B ∧
        (CompleteBetween G X A ↔ CompleteBetween G X B)) := by
...
theorem homogeneous_singleton (a b : V) :
    Lax768004.TwinWidth.Homogeneous G {a} {b} := by
...
def submittedTreeDecompositionOfSource
    ...
    Lax768004.Treewidth.TreeDecomposition G where
  Node := D.Node
...
theorem treewidth_le_of_source
    ...
    (D : TwinWidth.SimpleGraph.TreeDecomposition G)
    (hwidth : D.width ≤ w) :
    Lax768004.Treewidth.treewidth G ≤ w := by
...
def Faithful (G : SimpleGraph V) (T : TwinWidth.TrigraphState V) : Prop :=
  ∀ ⦃A B⦄, A ∈ T.bags → B ∈ T.bags → A ≠ B →
    ...
    (T.redAdj A B ↔ ¬ Lax768004.TwinWidth.Homogeneous G A B)
...
theorem contractedRed_iff [DecidableEq V]
    ...
    TwinWidth.SimpleGraph.contractedRed T A B X Y ↔
      ¬ Lax768004.TwinWidth.Homogeneous G X Y := by
...
theorem redDegree_eq_of_faithful [DecidableEq V]
    ...
    Lax768004.TwinWidth.redDegree G T.bags A = T.redDegree A := by
  classical
  unfold Lax768004.TwinWidth.redDegree TwinWidth.TrigraphState.redDegree
  rw [show {B | B ∈ T.bags ∧ B ≠ A ∧ ¬ Lax768004.TwinWidth.Homogeneous G A B} =
...
def submittedOfSource [Fintype V] [DecidableEq V] {d : ℕ}
    (S : TwinWidth.SimpleGraph.ContractionSequence G d) :
    Lax768004.TwinWidth.PartitionSequence G d where
...
theorem isPartitionFamily_singletonPartition [Fintype V] [DecidableEq V] :
    IsPartitionFamily (Lax768004.TwinWidth.singletonPartition V) := by
  refine ⟨?_, ?_, ?_⟩
  · intro A hA
...
theorem isPartitionFamily_partition [Fintype V] [DecidableEq V] {d : ℕ}
    (S : Lax768004.TwinWidth.PartitionSequence G d) :
...
def stateOf (G : SimpleGraph V) (P : Finset (Finset V))
  ...
  redAdj A B := A ≠ B ∧ ¬ Lax768004.TwinWidth.Homogeneous G A B
...
def clampedPartition [Fintype V] [DecidableEq V] {d : ℕ}
    (S : Lax768004.TwinWidth.PartitionSequence G d) (i : ℕ) :
    Finset (Finset V) :=
...
def sourceOfSubmitted [Fintype V] [DecidableEq V] {d : ℕ}
  ...
  starts := by
    have h0 : clampedPartition S 0 = Lax768004.TwinWidth.singletonPartition V := by
      rw [clampedPartition_of_le S (Nat.zero_le _), S.starts]
    refine ⟨h0, ?_, ?_⟩
    · intro A B hA hB
      have hA' : A ∈ Lax768004.TwinWidth.singletonPartition V := h0 ▸ hA
      have hB' : B ∈ Lax768004.TwinWidth.singletonPartition V := h0 ▸ hB
  ...
  redDegree_le i hi A hA := by
    ...
    rw [← hred]
    show Lax768004.TwinWidth.redDegree G (clampedPartition S i) A ≤ d
...
theorem hasTwinWidthAtMost_mono [Fintype V] [DecidableEq V] {d e : ℕ}
    (h : Lax768004.TwinWidth.HasTwinWidthAtMost G d) (hde : d ≤ e) :
    Lax768004.TwinWidth.HasTwinWidthAtMost G e := by
  obtain ⟨S⟩ := h
  exact ⟨{ S with
...
theorem lt_twinWidth_of_not_hasTwinWidthAtMost
    ...
    (hnot : ¬ Lax768004.TwinWidth.HasTwinWidthAtMost G d)
    (hex : ∃ e, Lax768004.TwinWidth.HasTwinWidthAtMost G e) :
    d < Lax768004.TwinWidth.twinWidth G := by
  have hne : {e | Lax768004.TwinWidth.HasTwinWidthAtMost G e}.Nonempty := hex
  have hmem : Lax768004.TwinWidth.HasTwinWidthAtMost G
      (Lax768004.TwinWidth.twinWidth G) := Nat.sInf_mem hne
  by_contra hle
...
theorem homogeneous_image {V V' : Type} {G : SimpleGraph V}
    {G' : SimpleGraph V'} [DecidableEq V'] (e : G ≃g G') (A B : Finset V) :
    Lax768004.TwinWidth.Homogeneous G' (A.image e.toEquiv) (B.image e.toEquiv) ↔
      Lax768004.TwinWidth.Homogeneous G A B := by
...
theorem image_singletonPartition {V V' : Type} [Fintype V] [DecidableEq V]
    ...
    (Lax768004.TwinWidth.singletonPartition V).image (Finset.image φ) =
      Lax768004.TwinWidth.singletonPartition V' := by
  ext A
  simp only [Lax768004.TwinWidth.singletonPartition, Finset.mem_image]
  constructor
  · rintro ⟨B, ⟨v, -, rfl⟩, rfl⟩
...
theorem redDegree_image {V V' : Type} [DecidableEq V] [DecidableEq V']
    ...
    Lax768004.TwinWidth.redDegree G' (P.image (Finset.image e.toEquiv))
        (A.image e.toEquiv) =
      Lax768004.TwinWidth.redDegree G P A := by
  unfold Lax768004.TwinWidth.redDegree
  ...
  rw [show {B | B ∈ P.image (Finset.image e.toEquiv) ∧
        B ≠ A.image e.toEquiv ∧
        ¬ Lax768004.TwinWidth.Homogeneous G' (A.image e.toEquiv) B} =
      Finset.image e.toEquiv ''
        {B | B ∈ P ∧ B ≠ A ∧ ¬ Lax768004.TwinWidth.Homogeneous G A B} from ?_]
...
/--
---
conclusion: Lax768004.ExponentialSeparation.exists_treewidth_le_and_two_pow_lt_twinWidth
---
...
theorem exists_treewidth_le_and_two_pow_lt_twinWidth (k : ℕ) :
    ∃ n : ℕ, ∃ G : SimpleGraph (Fin n),
      Lax768004.Treewidth.treewidth G ≤ 2 * k + 4 ∧
        2 ^ k < Lax768004.TwinWidth.twinWidth G := by
  classical
  ...
  · have hnot : ¬ Lax768004.TwinWidth.HasTwinWidthAtMost G' (2 ^ k) := by
      rintro ⟨S'⟩
      ...
      exact
        TwinWidth.SimpleGraph.BonnetDepres.bonnetDepres_not_hasTwinWidthAtMost_two_pow
          k hsource
    have hex : ∃ e, Lax768004.TwinWidth.HasTwinWidthAtMost G' e := by
