...
def ProfFillInv (oa ta xb vt nN nS : String) {N : ℕ} (ns : ℕ)
...
theorem vsFillTurn_step :
    ∀ σ, ProfFillInv oa ta xb vt nN nS ns off tgt X σ → σ.vars "pw.j" < ns →
      ∃ σ' K', Run B (vsFillTurn oa ta xb vt nN) σ σ' K' ∧
        ProfFillInv oa ta xb vt nN nS ns off tgt X σ' ∧
...
theorem vsFillCom_spec :
    Spec B (ProfFillInv oa ta xb vt nN nS ns off tgt X)
      (Csr.scan "pw.j" nS (vsFillTurn oa ta xb vt nN))
      (fun _ σ' => ProfFillInv oa ta xb vt nN nS ns off tgt X σ' ∧
        σ'.vars "pw.j" = ns)
      (21 * ns + 32 * N + 4) := by
  refine Csr.ownerScan_spec B (21 * ns + 32 * N + 4) N ns 17 28
    "pw.j" nS "pw.u" (vsFillTurn oa ta xb vt nN)
    (ProfFillInv oa ta xb vt nN nS ns off tgt X) (by omega)
...
theorem vsrcCom_spec
  ...
  -- 5. the fill
  have hFill7 : ProfFillInv oa ta xb vt nN nS ns off tgt X σ₇ := by
