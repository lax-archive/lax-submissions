...
/-!
# F6c12 (residual 4-i-a) — the frontier loop, and `PeelBfsIn` discharged

`SolveSweepBfs` owns one centre's turn as a *program* (`bfsTurnCom`)
together with everything the turn must produce (§5 there), may spend
(§3, §4) and may touch (§6). What it left open — and said so — is the
induction over levels: that the list the passes build **is** the ball,
with no duplicate and nothing extra, and with it the `Run` accounting
that turns `abf`, `bbf`, `cbf` from figures read off the program text
into proved budgets. This file is that induction, and
`peelBfsIn_bfsTurnCom` (§9) discharges `PeelBfsIn` at

    abf = 12·R + 44,   bbf = 39,   cbf = 76,

the sibling's three figures, confirmed. `PeelBfsIn` was the last
residual on the critical path to `CovPeelIn`.

## The induction

The reached list is `lm[b .. b + bf.c)` with `b = lo["sw.i"]`; `BfsRow`
(§1) pins it — duplicate-free, laid out in `lm`, with `co` its indicator
on the carrier, and with the two reference clauses (`co` outside the
carrier, `lm` below the anchor) that let the pass hand its scratch back.
`CaRow` (§3) is the assignment array as the marking passes leave it,
stated without an `if` so that its two clauses *are*
`ctrPart_succ_of_ball`'s `hhit` and `hkeep`.

One level pass (§5, `bfsLevel_run`) expands the entries below the frozen
end `bf.e` and appends the unvisited neighbours it finds, so

    set (row after the pass) = set (row before) ∪ N(the frontier),

and `ball_succ_frontier` (§6) is the geometric step that makes that one
radius: expanding only the *frontier* suffices, because the layer below
it had its neighbours pushed by the previous pass. `bfsIters_run` runs
that up to `k` passes at the shifted ball family `BlS` (`BlS 0 = ∅`,
`BlS (m+1) = ball H m u`), so the marking phase leaves the row at
`ball H R u` and the plain phase at `ball H (2R) u` — which is the
cluster (`cluster_eq_ball_peelSet`) — while the set the passes *expand*
is `ball H (2R-1) u`, exactly what `bfsExpanded_le_edgeTerm` prices.
...
than a count of pushes, and what makes the expanded list's degree sum a
sum over a *set* of vertices.

## The three constants, now proved

`bfs_cost_le` (§8) is the arithmetic, and every figure in it is a
literal `Com` size at the `Run` cost model:
...
* `bbf = 39` — `25` per expanded vertex (its row read, the head bump,
  the level loop's turn, the scan's prologue and exit) and `14` per
  reached vertex in the clean-up. A vertex at distance exactly `2·R`
  pays only the `14`.
* `cbf = 76 = 2·38` — `38` is one adjacency cell in a *marking* level
  pass; a plain pass costs `31` and is charged at `38` throughout (the
  push lemma takes its plain `13` at the marking `20`). The `2` is
  `sum_induced_deg_le_two_sum_dlt`'s.

The two program constraints bind as `SolveSweepBfs` said they would.
The BFS does not expand its final level — structurally, since
`bfsTurnCom` is `2·R` guard-free passes — and the induction *shows* it:
the expanded prefix's set is `BlS (2R) = ball H (2R-1) u`. The marks are
cleared by re-walking the row, at `bfsClear_spec`'s `14·cnt + 6`; the
carrier is never scanned.

## The scratch descriptor

`Ssc` is chosen here as `BfsClean (co j) n` — "the visited flags are
clear" — and it comes back out. No allocation clause is added to it
(Hazard 6): the clean-up is instantiated at `min n |co|`, and the cells
above the carrier are handled by `BfsRow`'s own reference clause rather
than by asking the caller for more space.
-/
...
def BfsScanInv (ca co lm : String) {N : ℕ} (base : ℕ) (c₀ l₀ : ℕ → ℕ) (g : Fin N → ℕ)
...
theorem bfsScan_run {ca co ao aj dg lo lm nn : String} (mark : Bool)
  ...
  have hbody : Spec B
      (fun σ => BfsScanInv ca co lm base c₀ l₀ g uval mark M X f L₀ o n σ₀ σ ∧
        σ.vars "bf.t" < n)
      ...
      (fun σ σ' => BfsScanInv ca co lm base c₀ l₀ g uval mark M X f L₀ o n σ₀ σ' ∧
  ...
  have hloop := Spec.forRangeZero (B := B) "bf.t" "bf.n"
    (BfsScanInv ca co lm base c₀ l₀ g uval mark M X f L₀ o n σ₀) n 34 hnB
    (fun _ hσ => by obtain ⟨-, -, -, -, -, -, -, -, h, -, -, -⟩ := hσ; exact h)
    (fun _ hσ => by obtain ⟨-, -, -, -, -, -, -, h, -, -, -, -⟩ := hσ; exact h)
    hbody
  have hentry : BfsScanInv ca co lm base c₀ l₀ g uval mark M X f L₀ o n σ₀
      ((ρ.setVar "bf.t" 0)) := by
...
theorem peelBfsIn_bfsTurnCom (C : GraphClass) (hC : NowhereDense C) (φ : FO 0)
  ...
  obtain ⟨hAW, hordr, hcoLen, hlmLen, hSsc, hdel, hctr, hlog⟩ := id hst
  ...
  -- §1 the initial push
  have hilo : i < (σ.arrs (lo j)).length := by omega
  have hr1 : Run B (.assign "bf.b" (.get (lo j) (.var "sw.i"))) σ
      (σ.setVar "bf.b" base) (1 + 2) := by
    have hg : (Expr.get (lo j) (.var "sw.i")).evalB B σ = some base := by
      refine evalB_get (k := i) ?_ ?_ hbaseB
      · have := evalB_var (B := B) (x := "sw.i") (σ := σ) (by rw [hswi]; omega)
        rw [hswi] at this; exact this
      · rw [getElem?_of_lt hilo, hlobase]
    have := Run.assign (B := B) (σ := σ) (x := "bf.b") hg
    simpa using this
  set ρ1 : Env := σ.setVar "bf.b" base with hρ1
  have hr2 : Run B (.assign "bf.c" (.lit 0)) ρ1 (ρ1.setVar "bf.c" 0) (1 + 1) := by
    have := Run.assign (B := B) (σ := ρ1) (x := "bf.c")
      (evalB_lit (n := 0) (B := B) (by omega))
    simpa using this
  set ρ2 : Env := ρ1.setVar "bf.c" 0 with hρ2
  have hr3 : Run B (.assign "bf.h" (.lit 0)) ρ2 (ρ2.setVar "bf.h" 0) (1 + 1) := by
    have := Run.assign (B := B) (σ := ρ2) (x := "bf.h")
      (evalB_lit (n := 0) (B := B) (by omega))
    simpa using this
  set ρ3 : Env := ρ2.setVar "bf.h" 0 with hρ3
  have hρ3sv : ρ3.vars "sw.v" = ((u : Fin A.N) : ℕ) := by
    rw [hρ3, vars_setVar, if_neg (by decide), hρ2, vars_setVar, if_neg (by decide),
      hρ1, vars_setVar, if_neg (by decide), hswv]
  have hr4 : Run B (.assign "bf.w" (.var "sw.v")) ρ3
      (ρ3.setVar "bf.w" ((u : Fin A.N) : ℕ)) (1 + 1) := by
    have hv : (Expr.var "sw.v").evalB B ρ3 = some ((u : Fin A.N) : ℕ) := by
      rw [← hρ3sv]; exact evalB_var (by rw [hρ3sv]; omega)
    have := Run.assign (B := B) (σ := ρ3) (x := "bf.w") hv
    simpa using this
  set ρ4 : Env := ρ3.setVar "bf.w" ((u : Fin A.N) : ℕ) with hρ4
  have hρ4a : ∀ b, ρ4.arrs b = σ.arrs b := by
    intro b; rw [hρ4, arrs_setVar, hρ3, arrs_setVar, hρ2, arrs_setVar, hρ1, arrs_setVar]
  have hρ4v : ∀ y, y ≠ "bf.b" → y ≠ "bf.c" → y ≠ "bf.h" → y ≠ "bf.w" →
      ρ4.vars y = σ.vars y := by
    intro y h1 h2 h3 h4
    rw [hρ4, vars_setVar, if_neg h4, hρ3, vars_setVar, if_neg h3, hρ2, vars_setVar,
      if_neg h2, hρ1, vars_setVar, if_neg h1]
  have hrow0 : BfsRow (co j) (lm j) base c₀ l₀ ([] : List (Fin A.N)) ρ4 := by
    refine ⟨?_, ?_, List.nodup_nil, ?_, ?_, ?_, ?_, ?_, ?_⟩
    · rw [hρ4, vars_setVar, if_neg (by decide), hρ3, vars_setVar, if_neg (by decide),
        hρ2, vars_setVar, if_neg (by decide), hρ1, vars_setVar, if_pos rfl]
    · rw [hρ4, vars_setVar, if_neg (by decide), hρ3, vars_setVar, if_neg (by decide),
        hρ2, vars_setVar, if_pos rfl]
      simp
    · rw [hρ4a]; simpa using le_trans (Nat.le_add_right base _) hroom
    · intro t ht; simp at ht
    · rw [hρ4a]; omega
    · intro v
      rw [hρ4a, if_neg (by simp)]
      exact hSsc (v : ℕ) (lt_of_lt_of_le v.isLt hAN)
    · intro v _; rw [hρ4a]
    · intro m _; rw [hρ4a]
  have hca0 : CaRow (ca j) g ((u : Fin A.N) : ℕ) ∅ ρ4 := by
    refine ⟨by rw [hρ4a]; exact hcaLen, fun z hz => absurd hz (by simp), ?_⟩
    intro z _; rw [hρ4a]
  obtain ⟨τ5, hrp, hrow5, hv5, ha5, hl5, -, hca5⟩ :=
    bfsPush_run (ca := ca j) (co := co j) (lm := lm j) (nn := (arenaNames j).nN)
      true hcolm hcaco hcalm hnc (N := A.N) (base := base) (B := B) (c₀ := c₀)
      (l₀ := l₀) (L := ([] : List (Fin A.N))) (w := u) (σ := ρ4) hrow0
      (by rw [hρ4, vars_setVar, if_pos rfl]) (by simp)
      (by
        rw [hρ4a]
        have := Set.ncard_pos (s := cluster St A π u)
        have h2 : 0 < (cluster St A π u).ncard :=
          Set.ncard_pos (Set.toFinite _) |>.mpr ⟨u, huX⟩
        simp only [List.length_nil, Nat.add_zero]
        omega)
      (by rw [hρ4a]; exact hcaLen)
      (by intro z; rw [hρ4a]; exact hgN z)
      (by rw [hρ4v _ hnb' hnc hnh hnw, hnnv])
      (by rw [hρ4v _ (by decide) (by decide) (by decide) (by decide), hswv]; exact huN)
      hANB
      (by
        have h2 : 0 < (cluster St A π u).ncard :=
          Set.ncard_pos (Set.toFinite _) |>.mpr ⟨u, huX⟩
        simp only [List.length_nil, Nat.add_zero]
        omega)
  have hcaτ5 : CaRow (ca j) g ((u : Fin A.N) : ℕ) {z | z ∈ ([u] : List (Fin A.N))} τ5 := by
    refine ⟨by rw [hl5 (ca j)]; exact hca0.1, ?_, ?_⟩
    · intro z hz hgz
      have hzu : z = u := by simpa using hz
      have hgu : ¬ (g u < A.N) := by rw [← hzu]; exact hgz
      rw [hca5 z, if_pos ⟨rfl, hzu, by rw [hρ4a]; exact hgu⟩,
        hρ4v _ (by decide) (by decide) (by decide) (by decide)]
      exact hswv
    · intro z hz
      rw [hca5 z]
      by_cases hzu : z = u
      · have hgz : g z < A.N := by
          rcases hz with h | h
          · refine absurd ?_ h
            show z ∈ ({y : Fin A.N | y ∈ ([u] : List (Fin A.N))} : Set (Fin A.N))
            simp [hzu]
          · exact h
        have hgu : g u < A.N := by rw [← hzu]; exact hgz
        rw [if_neg (by rintro ⟨-, -, hcc⟩; exact hcc (by rw [hρ4a]; exact hgu)), hρ4a]
      · rw [if_neg (by simp [hzu]), hρ4a]
  have hrunInit : Run B (bfsInitCom (ca j) (co j) (lo j) (lm j) (arenaNames j).nN)
      σ τ5 29 := by
    rw [bfsInitCom]
    exact ((hr1.seq (hr2.seq (hr3.seq (hr4.seq hrp))))).mono (by omega)
  -- §2 the marking passes
  have hτ5a : ∀ b, b ≠ co j → b ≠ lm j → b ≠ ca j → τ5.arrs b = σ.arrs b := by
    intro b h1 h2 h3; rw [ha5 b h1 h2 h3, hρ4a]
  have hτ5v : ∀ y, y ≠ "bf.b" → y ≠ "bf.c" → y ≠ "bf.h" → y ≠ "bf.w" →
      τ5.vars y = σ.vars y := by
    intro y h1 h2 h3 h4; rw [hv5 y h2, hρ4v y h1 h2 h3 h4]
  obtain ⟨τ6, L₁, E₁, hrun6, hrow6, hca6, hLX6, hpre6, hmem6, htake6, hseg6, hhle6,
    hfrv6, hfra6, hfrl6⟩ :=
    bfsIters_run (ca := ca j) (co := co j) (ao := ao j) (aj := aj j) (dg := dg j)
      (lo := lo j) (lm := lm j) (nn := (arenaNames j).nN) true (hnm j) hnnf
      (N := A.N) (base := base) (B := B) (c₀ := c₀) (l₀ := l₀) (g := g)
      (uval := ((u : Fin A.N) : ℕ)) (X := cluster St A π u) (H := Hc) (u := u)
      (deg := degf) hANB hcapB (fun z => hgN z) huN
      St.R 0 [u] 0 {z | z ∈ ([u] : List (Fin A.N))} τ5
      hrow5 hcaτ5
      (hadj.of_eq (hτ5a (ao j) haoco haolm haoca) (hτ5a (aj j) hajco hajlm hajca)
        (hτ5a (dg j) hdgco hdglm hdgca))
      (by intro z hz; rw [List.mem_singleton.mp hz]; exact huX)
      (by rw [hl5 (lm j), hρ4a]; exact hroom)
      (by intro z hz; exact hz) (fun _ z hz => hz)
      (by
        intro v hv z hadjvz
        exact hexpX (0 + St.R) (by omega) v hv z hadjvz)
      (by
        rw [hv5 "bf.h" (by decide), hρ4, vars_setVar, if_neg (by decide), hρ3,
          vars_setVar, if_pos rfl])
      (by simp)
      (by intro z; simp)
      (by
        intro z
        rw [show (0 : ℕ) + 1 = 1 from rfl, BlS_succ, ball_zero]
        simp)
      (by rw [hτ5v _ hnb' hnc hnh hnw, hnnv])
      (by rw [hτ5v _ (by decide) (by decide) (by decide) (by decide), hswv])
  -- §3 the plain passes
  have hτ6a : ∀ b, b ≠ co j → b ≠ lm j → b ≠ ca j → τ6.arrs b = σ.arrs b := by
    intro b h1 h2 h3; rw [hfra6 b h1 h2 h3, hτ5a b h1 h2 h3]
  have hτ6l : ∀ b, (τ6.arrs b).length = (σ.arrs b).length := by
    intro b; rw [hfrl6 b, hl5 b, hρ4a]
  have hupre : ∀ z : Fin A.N, z ∈ ([u] : List (Fin A.N)) → z ∈ L₁ := by
    obtain ⟨D, hD⟩ := hpre6
    intro z hz; rw [hD]; exact List.mem_append.mpr (Or.inl hz)
  have hca6' : CaRow (ca j) g ((u : Fin A.N) : ℕ) {z | z ∈ L₁} τ6 := by
    have hMeq : (if (true : Bool) = true then
        ({z | z ∈ ([u] : List (Fin A.N))} : Set (Fin A.N)) ∪ {z | z ∈ L₁}
        else {z | z ∈ ([u] : List (Fin A.N))}) = {z | z ∈ L₁} := by
      rw [if_pos rfl]
      ext z
      simp only [Set.mem_union, Set.mem_setOf_eq]
      exact ⟨fun h => h.elim (fun h' => hupre z h') id, fun h => Or.inr h⟩
    rw [hMeq] at hca6
    exact hca6
  obtain ⟨τ7, L₂, E₂, hrun7, hrow7, hca7, hLX7, hpre7, hmem7, htake7, hseg7, hhle7,
    hfrv7, hfra7, hfrl7⟩ :=
    bfsIters_run (ca := ca j) (co := co j) (ao := ao j) (aj := aj j) (dg := dg j)
      (lo := lo j) (lm := lm j) (nn := (arenaNames j).nN) false (hnm j) hnnf
      (N := A.N) (base := base) (B := B) (c₀ := c₀) (l₀ := l₀) (g := g)
      (uval := ((u : Fin A.N) : ℕ)) (X := cluster St A π u) (H := Hc) (u := u)
      (deg := degf) hANB hcapB (fun z => hgN z) huN
      St.R St.R L₁ (τ6.vars "bf.h") {z | z ∈ L₁} τ6
      hrow6 hca6'
      (hadj.of_eq (hτ6a (ao j) haoco haolm haoca) (hτ6a (aj j) hajco hajlm hajca)
        (hτ6a (dg j) hdgco hdglm hdgca))
      hLX6 (by rw [hτ6l (lm j)]; exact hroom)
      (fun z hz => hz) (by intro hm; exact absurd hm (by decide))
      (by intro v hv z hadjvz; exact hexpX (St.R + St.R) (by omega) v hv z hadjvz)
      rfl hhle6 (by intro z; rw [htake6 z, Nat.zero_add])
      (by intro z; rw [hmem6 z, show (0 : ℕ) + St.R + 1 = St.R + 1 from by omega])
      (by rw [hfrv6 _ hnc hno hnn2 hnt hnw hne hnh hnv, hτ5v _ hnb' hnc hnh hnw, hnnv])
      (by
        rw [hfrv6 "sw.v" (by decide) (by decide) (by decide) (by decide) (by decide)
          (by decide) (by decide) (by decide),
          hτ5v "sw.v" (by decide) (by decide) (by decide) (by decide), hswv])
  have hca7' : CaRow (ca j) g ((u : Fin A.N) : ℕ) {z | z ∈ L₁} τ7 := by
    rw [if_neg (by decide)] at hca7
    exact hca7
  have hτ7a : ∀ b, b ≠ co j → b ≠ lm j → b ≠ ca j → τ7.arrs b = σ.arrs b := by
    intro b h1 h2 h3; rw [hfra7 b h1 h2 h3, hτ6a b h1 h2 h3]
  have hτ7l : ∀ b, (τ7.arrs b).length = (σ.arrs b).length := by
    intro b; rw [hfrl7 b, hτ6l b]
  have hτ7v : ∀ y, y ≠ "bf.b" → y ≠ "bf.c" → y ≠ "bf.o" → y ≠ "bf.n" → y ≠ "bf.t" →
      y ≠ "bf.w" → y ≠ "bf.e" → y ≠ "bf.h" → y ≠ "bf.v" → τ7.vars y = σ.vars y := by
    intro y h0 h1 h2 h3 h4 h5 h6 h7 h8
    rw [hfrv7 y h1 h2 h3 h4 h5 h6 h7 h8, hfrv6 y h1 h2 h3 h4 h5 h6 h7 h8,
      hτ5v y h0 h1 h7 h5]
  -- the emitted row is the cluster
  have hL₂X : ∀ z : Fin A.N, z ∈ L₂ ↔ z ∈ cluster St A π u := by
    intro z
    rw [hmem7 z, hXball, show St.R + St.R + 1 = 2 * St.R + 1 from by omega, BlS_succ]
  have hcnt : L₂.length = (cluster St A π u).ncard :=
    length_eq_ncard_of_nodup hrow7.nodup hL₂X
  -- §4 the offset write
  obtain ⟨τ8, hrun8, hτ8eq⟩ :=
    (bfsEmit_spec (lo := lo j) (B := B) (i := i) (base := base) (cnt := L₂.length)
      (by omega) h1B (by omega)).run
      ⟨by rw [hτ7v "sw.i" (by decide) (by decide) (by decide) (by decide) (by decide)
            (by decide) (by decide) (by decide) (by decide)]; exact hswi,
        hrow7.b, hrow7.c, by rw [hτ7l (lo j)]; omega⟩
  -- §5 the clean-up
  obtain ⟨τ9, hrun9, hclean, -, -⟩ :=
    (bfsClear_spec (co := co j) (lm := lm j) hcolm (B := B)
      (N := min n (σ.arrs (co j)).length) (base := base) (cnt := L₂.length)
      (row := fun k => if h : k < L₂.length then ((L₂[k] : Fin A.N) : ℕ) else 0)
      (by omega) (by omega) h1B).run (σ := τ8) (by
        have hco8 : τ8.arrs (co j) = τ7.arrs (co j) := by
          rw [hτ8eq, arrs_setArr, if_neg hcolo]
        have hlm8 : τ8.arrs (lm j) = τ7.arrs (lm j) := by
          rw [hτ8eq, arrs_setArr, if_neg hlmlo]
        refine ⟨?_, ?_, by rw [vars_setVar, if_pos rfl]; omega, ?_, ?_, ?_, ?_⟩
        · rw [vars_setVar, if_neg (by decide), hτ8eq, vars_setArr]; exact hrow7.b
        · rw [vars_setVar, if_neg (by decide), hτ8eq, vars_setArr]; exact hrow7.c
        · rw [arrs_setVar, hco8, hτ7l (co j)]; omega
        · intro k hk
          rw [arrs_setVar, hlm8]
          refine ⟨by have := hrow7.fits; omega, ?_⟩
          rw [hrow7.cell hk]
          simp only [dif_pos hk]
        · intro k hk
          simp only [dif_pos hk]
          have := (L₂[k] : Fin A.N).isLt
          omega
        · intro v hv
          rw [arrs_setVar, hco8, vars_setVar, if_pos rfl]
          by_cases hvN : v < A.N
          · have hco := hrow7.mark ⟨v, hvN⟩
            by_cases hmem : (⟨v, hvN⟩ : Fin A.N) ∈ L₂
            · obtain ⟨k, hk, hkv⟩ := List.mem_iff_getElem.mp hmem
              exact Or.inr ⟨k, Nat.zero_le _, hk, by simp only [dif_pos hk]; rw [hkv]⟩
            · left; rw [hco, if_neg hmem]
          · left
            rw [hrow7.outside (by omega)]
            exact hSsc v (by omega))
  -- §6 the whole turn
  have hEseg : L₂.take (τ7.vars "bf.h") = E₁ ++ E₂ := by
    rw [hseg7, hseg6]
    simp
  have hEnd : (L₂.take (τ7.vars "bf.h")).Nodup :=
    List.Nodup.sublist (List.take_sublist _ _) hrow7.nodup
  have hEmem : ∀ z : Fin A.N, z ∈ L₂.take (τ7.vars "bf.h") ↔
      z ∈ ball Hc (2 * St.R - 1) u := by
    intro z
    rw [htake7 z, show St.R + St.R = (2 * St.R - 1) + 1 from by omega, BlS_succ]
  have hKle : 29 + ((1 + 6 * St.R + (E₁.map (fun v => 25 + 38 * degf v)).sum)
        + ((1 + 6 * St.R + (E₂.map (fun v => 25 + 38 * degf v)).sum)
          + (7 + (14 * L₂.length + 6))))
      ≤ peelTurn St A π (12 * St.R + 44) 39 76 i := by
    have hc : 44 + 12 * St.R
        + ((L₂.take (τ7.vars "bf.h")).map (fun v => 25 + 38 * degf v)).sum
        + 14 * L₂.length ≤ peelTurn St A π (12 * St.R + 44) 39 76 i :=
      bfs_cost_le St A π u hR hi hiu (L₂.take (τ7.vars "bf.h")) L₂ hEnd
        hrow7.nodup hEmem hL₂X
    rw [hEseg, List.map_append, List.sum_append] at hc
    omega
  have hrunTurn : Run B (bfsTurnCom St.R (ca j) (co j) (ao j) (aj j) (dg j) (lo j)
      (lm j) (arenaNames j).nN) σ τ9
      (peelTurn St A π (12 * St.R + 44) 39 76 i) := by
    rw [bfsTurnCom]
    exact ((hrunInit.seq (hrun6.seq (hrun7.seq (hrun8.seq hrun9))))).mono hKle
  -- §7 the loop state, one rank on
  have hwarr : ∀ b : String, b ≠ ca j → b ≠ co j → b ≠ lo j → b ≠ lm j →
      τ9.arrs b = σ.arrs b := by
    intro b h1 h2 h3 h4
    exact hrunTurn.frame_arr b (bfsTurn_warrs St.R (ca j) (co j) (ao j) (aj j) (dg j)
      (lo j) (lm j) (arenaNames j).nN h1 h2 h3 h4)
  have hwvar : ∀ y : String, y ≠ "bf.b" → y ≠ "bf.c" → y ≠ "bf.h" → y ≠ "bf.e" →
      y ≠ "bf.v" → y ≠ "bf.o" → y ≠ "bf.n" → y ≠ "bf.t" → y ≠ "bf.w" → y ≠ "bf.k" →
      τ9.vars y = σ.vars y := by
    intro y h1 h2 h3 h4 h5 h6 h7 h8 h9 h10
    exact hrunTurn.frame_var y (bfsTurn_wvars St.R (ca j) (co j) (ao j) (aj j) (dg j)
      (lo j) (lm j) (arenaNames j).nN h1 h2 h3 h4 h5 h6 h7 h8 h9 h10)
  have hlen9 : ∀ b, (τ9.arrs b).length = (σ.arrs b).length :=
    fun b => Run.arrs_length hrunTurn b
  have hca9 : τ9.arrs (ca j) = τ7.arrs (ca j) := by
    rw [hrun9.frame_arr (ca j) (by simp [bfsClearCom, Com.warrs, hcaco]), hτ8eq,
      arrs_setArr, if_neg hcalo]
  have hlo9 : τ9.arrs (lo j) = (σ.arrs (lo j)).set (i + 1) (base + L₂.length) := by
    rw [hrun9.frame_arr (lo j) (by simp [bfsClearCom, Com.warrs, Ne.symm hcolo]),
      hτ8eq, arrs_setArr, if_pos rfl,
      hτ7a (lo j) (Ne.symm hcolo) (Ne.symm hlmlo) (Ne.symm hcalo)]
  have hlm9 : τ9.arrs (lm j) = τ7.arrs (lm j) := by
    rw [hrun9.frame_arr (lm j) (by simp [bfsClearCom, Com.warrs, Ne.symm hcolm]),
      hτ8eq, arrs_setArr, if_neg hlmlo]
  refine ⟨τ9, hrunTurn, ?_, ?_, ?_⟩
  · refine sweepSt_step_of_bfs hst
      (hwarr _ ((hfr j _ (Or.inl rfl)).1) ((hfr j _ (Or.inl rfl)).2.1)
        ((hfr j _ (Or.inl rfl)).2.2.1) ((hfr j _ (Or.inl rfl)).2.2.2))
      (hwarr _ ((hfr j _ (Or.inr (Or.inl rfl))).1)
        ((hfr j _ (Or.inr (Or.inl rfl))).2.1)
        ((hfr j _ (Or.inr (Or.inl rfl))).2.2.1)
        ((hfr j _ (Or.inr (Or.inl rfl))).2.2.2))
      (hwarr _ ((hfr j _ (Or.inr (Or.inr (Or.inl rfl)))).1)
        ((hfr j _ (Or.inr (Or.inr (Or.inl rfl)))).2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inl rfl)))).2.2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inl rfl)))).2.2.2))
      (hwarr _ ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inl rfl))))).1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inl rfl))))).2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inl rfl))))).2.2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inl rfl))))).2.2.2))
      (hwarr _ ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))).1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))).2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))).2.2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))).2.2.2))
      (hwvar _ hnb' hnc hnh hne hnv hno hnn2 hnt hnw hnk)
      ?_
      (hwarr _ ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))))).1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))))).2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))))).2.2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))))).2.2.2))
      (hwarr _ haoca haoco haolo haolm) (hwarr _ hajca hajco hajlo hajlm)
      (hwarr _ hdgca hdgco hdglo hdglm)
      (hwarr _ ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr rfl))))))).1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr rfl))))))).2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr rfl))))))).2.2.1)
        ((hfr j _ (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr rfl))))))).2.2.2))
      (hlen9 (co j)) (by rw [hlen9 (lm j)]; exact hlmLen) ?_ ?_ ?_
    · exact hwvar _ ((arenaScalars_ne j (by decide) (by decide) (by decide)).2)
        ((arenaScalars_ne j (by decide) (by decide) (by decide)).2)
        ((arenaScalars_ne j (by decide) (by decide) (by decide)).2)
        ((arenaScalars_ne j (by decide) (by decide) (by decide)).2)
        ((arenaScalars_ne j (by decide) (by decide) (by decide)).2)
        ((arenaScalars_ne j (by decide) (by decide) (by decide)).2)
        ...
        ((arenaScalars_ne j (by decide) (by decide) (by decide)).2)
        ((arenaScalars_ne j (by decide) (by decide) (by decide)).2)
    · -- the scratch descriptor comes back
      intro v hv
      by_cases hvl : v < (σ.arrs (co j)).length
      · exact hclean v (by omega)
      · rw [List.getD_eq_getElem?_getD,
          List.getElem?_eq_none (by rw [hlen9 (co j)]; omega)]
        rfl
    · -- the marks
      refine ctrPart_succ_of_ball St A π hi hctr (by rw [hlen9 (ca j)]; exact hcaLen)
        ?_ ?_
      · intro v hv hgv
        rw [hca9]
        refine hca7'.2.1 v ?_ hgv
        show v ∈ L₁
        exact (hmem6 v).mpr (by
          rw [show (0 : ℕ) + St.R + 1 = St.R + 1 from by omega, BlS_succ]
          exact hv)
      · intro v hv
        rw [hca9]
        refine hca7'.2.2 v ?_
        ...
        · refine Or.inr (hv ?_)
          have := (hmem6 v).mp hvL
          rw [show (0 : ℕ) + St.R + 1 = St.R + 1 from by omega, BlS_succ] at this
          exact this
        · exact Or.inl hvL
    · -- the log
      refine logPart_succ_of_row St A π hi hlog rfl (cnt := L₂.length) ?_
        (by rw [hlen9 (lo j)]; exact hloLen) ?_ ?_ ?_ ?_ ?_ ?_
      · exact hcnt
      · intro k hk
        rw [hlo9, getD_set_of_ne (by omega)]
      · rw [hlo9, getD_set_self (by omega)]
      · rw [hlm9]
        have := hrow7.fits
        omega
      · intro m hm
        rw [hlm9]
        exact hrow7.below hm
      · intro t ht
        refine ⟨L₂[t], (hL₂X L₂[t]).mp (List.getElem_mem ht), ?_⟩
        rw [hlm9, hrow7.cell ht]
      · intro z hz
        obtain ⟨t, ht, hzt⟩ := List.mem_iff_getElem.mp ((hL₂X z).mpr hz)
        exact ⟨t, ht, by rw [hlm9, hrow7.cell ht, hzt]⟩
  · rw [hwvar "sw.i" (by decide) (by decide) (by decide) (by decide) (by decide)
      (by decide) (by decide) (by decide) (by decide) (by decide)]
    exact hswi
  · rw [hwvar "sw.v" (by decide) (by decide) (by decide) (by decide) (by decide)
      (by decide) (by decide) (by decide) (by decide) (by decide)]
    exact hswv

/-! The leaf's axiom profile: the three standard axioms, and the one
endorsed Lax12 axiom the setup carries. -/

#print axioms bfsScan_run

#print axioms bfsLevel_run

#print axioms bfsIters_run

#print axioms ball_succ_frontier

#print axioms adjRows_of_delAdjSt

#print axioms bfs_cost_le

#print axioms peelBfsIn_bfsTurnCom
