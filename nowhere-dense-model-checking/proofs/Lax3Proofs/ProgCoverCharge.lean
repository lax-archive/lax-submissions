...
`levelCharge D = 3n + 2·arcCount D + 4·fratPairCount D +
2·transPairCount D` allocates, stage by stage: the two enumerations'
per-vertex loop headers and the peel's bucket initialization (`3·n`);
the transitive enumeration's middle loop over the arcs and the copy of
the old arcs (`2·arcCount`); the fraternal enumeration, the fraternity
graph's adjacency build, the peel's edge work (`≤ 2·#edges ≤
fratPairCount`) and the fraternal decisions (`4·fratPairCount`); the
transitive candidates enumerated and decided (`2·transPairCount`).  The
base charge `m + 3·arcCount (D 0)` prices the peel of `G` itself plus
building `baseOr` (the arcs of `D 0` *are* the edges of `G`:
`baseOr_orients`).  The final charge `3·n + 5·arcCount (D R)` prices
symmetrizing `(D R).toGraph` (adjacency from the arcs, `≤ 2·arcs`
...
/-- **The charge of one greedy round** from `D`, stage by stage
(module docstring): the per-vertex loop headers of the two
enumerations and the peel's bucket initialization (`3·n`); the
transitive enumeration's middle loop and the old-arc copy
(`2·arcCount`); the fraternal enumeration, the fraternity graph's
adjacency build, its peel's edge work and the fraternal decisions
(`4·fratPairCount`); the transitive candidates, enumerated and decided
(`2·transPairCount`). -/
def levelCharge (D : Orientation n) : ℕ :=
  3 * n + 2 * arcCount D + 4 * fratPairCount D + 2 * transPairCount D
...
theorem levelCharge_le {D : Orientation n} {d : ℕ} (hd : D.InDegLE d) :
    levelCharge D ≤ n * (3 + 2 * d + 6 * (d * d)) := by
  have h1 := arcCount_le hd
  have h2 := fratPairCount_le hd
  have h3 := transPairCount_le hd
  calc levelCharge D
      = 3 * n + 2 * arcCount D + 4 * fratPairCount D + 2 * transPairCount D := rfl
    _ ≤ 3 * n + 2 * (n * d) + 4 * (n * (d * d)) + 2 * (n * (d * d)) := by omega
    _ = n * (3 + 2 * d + 6 * (d * d)) := by ring
...
theorem le_chainCharge {m : ℕ} (G : SimpleGraph (Fin m)) (R : ℕ) :
    m ≤ chainCharge G R := by
  have h1 : m ≤ baseCharge G := Nat.le_add_right m _
  have h2 : baseCharge G ≤ chainCharge G R :=
...
theorem chainCharge_le_of_uniform {m : ℕ} {G : SimpleGraph (Fin m)} {R d : ℕ}
    ...
    chainCharge G R ≤ (6 * R + 8) * (m * ((d + 1) * (d + 1))) := by
...
/-- `1 ≤ m^ε` for `m ≥ 1`, `ε ≥ 0`. -/
theorem one_le_rpow {m : ℕ} (hm : 0 < m) {ε : ℝ} (hε : 0 ≤ ε) :
    (1 : ℝ) ≤ (m : ℝ) ^ ε := by
...
theorem exists_chainCharge_le (C : GraphClass) (hC : NowhereDense C) (R : ℕ)
  ...
  have hsq : (dmax + 1) * (dmax + 1) ≤ 4 * ((3 * D₁ + 2) ^ (2 * 16 ^ R)) := by
    ...
    rwa [hdd] at h4
  have hN2 : chainCharge G R ≤ 4 * (6 * R + 8) * (m * (3 * D₁ + 2) ^ (2 * 16 ^ R)) := by
    calc chainCharge G R ≤ (6 * R + 8) * (m * ((dmax + 1) * (dmax + 1))) := hN
