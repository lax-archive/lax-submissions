...
/-!
# F6c12-5c-iii — the IMP+ text of the augmentation round's emit

`SolveAugEmit` (F6c12-5c-ii) proved **all the mathematics** of the
greedy round's body and left exactly two residuals open, both quantified
over a command and a budget: `TransposeIn` and `StepEmitIn`.  This file
supplies the missing loop text and discharges them.  Not one statement
about *which* finset a row holds is re-proved here: `emRow_eq_greedyStep`
is the row, `trInCsr_emit` turns any family that lists each `emRow D rk v`
once into `TrInCsr o' t' (greedyStep rk D)` with all ten clauses, and
`emK` with `emK_le_levelCharge` is the budget.  What is added is a
`Com`, its `Run` steps, and the invariants that carry them.

## The two programs

`tpCom` — **the transpose**, a counting sort in three sweeps:

* `tpCntCom` counts, one turn per slot of the input CSR: `dg[t[p]] += 1`,
  so `dg[u]` ends at `u`'s out-degree.  The extent `ns` is *not* in a
  named cell and does not have to be: it is `o[nN]`, one array read
  (`TrInCsr.offGet`), and the sweep's bound is that scalar.
* `tpOffCom` prefix-sums, one turn per vertex: `qo[i] := acc`,
  `dg[i] := acc`, `acc += deg` — the degree is read into a scalar before
  the cell is overwritten, so the same region serves as the degree
  counter and then as the scatter's cursor.
* `tpScatCom` scatters, one outer turn per head and one inner turn per
  slot: `qt[dg[t[j]]] := v` and bump.  The rows come out in increasing
  head order, but `OutCsrAt` asks only for the set, so no ordering claim
  is made.

`emCom` — **the round body**, one outer turn per head `v`:

* stamp the adjacency mark `ad[u] := v+1` over row `v` of the input and
  over row `v` of the transpose — that is `adjSet D v`, and `mem_adjSet`
  is that the stamp decides `D.Adjacent` in **both** directions;
* copy the old arcs into the output (fused with the first stamping
  sweep: the same slot is read once);
* the **fraternal phase** over row `v` of the fraternity CSR — stamp
  `sd[u] := v+1` and emit when `ad[u] ≠ v+1` and `sg[u] < sg[v]`
  (`EmFratPick`);
* the **transitive phase** over row `v` of `TransCsrAt` — emit when
  `sd[u] ≠ v+1`, `ad[u] ≠ v+1` and either `sg[u] < sg[v]` or
  `mk[u·n+v] = 0` (`EmTransPick`), the last being the single matrix read
  that decides the forced transitive direction.

Neither mark region is ever cleared.  Both are **row-stamped** with
`v+1`, and the invariant is that every cell holds a value `≤ v`; a stamp
left by an earlier head is therefore not the current head's stamp, and
the sweep that wrote it owes nothing.  The regions are asked to be zero
on entry, which is the machine's own initial state
(`Imp.lean:20-44`).

## Shape

Both contracts take the carrier size from the named scalar `nN` and the
fraternity slot count from `nF`, because **IMP+ reads no array length**
(`Imp.lean:158`).  Every index the emit forms is a word: the matrix index
`u·n + v` is below `n·n`, which is why the contract's word bound carries
an `n·n` term.

## The budget

`tpK n a = 41·n + 40·a + 30` and

    emK n a f T = 300·n + 300·a + 200·f + 240·T + 80

exactly as `SolveAugEmit` pinned it — the transpose is inside the `n`
and `a` terms with room to spare, and `emK_le_levelCharge` is unchanged.
The measured per-turn costs are in the docstrings of `tpK` and `emK'`.

## Findings

1. **The transpose's extent needs no cell.**  `TransposeIn` names only
   `nN`, and a counting sort looks like it needs the slot count too; but
   `ns = off n` is the last offset of the input CSR, so `o[nN]` is it.
   That is why the contract as landed is meetable with no extra cell —
   the one place in this campaign where the `AdjBuildIn` failure mode
   (`SolveSweepAdj.lean:308`) does *not* recur, and for a reason rather
   than by luck.
2. **The row stamps need no clearing sweep, and that is what keeps the
   round linear.**  A per-head `n`-cell clear would cost `n²` over the
   pass and break `emK`'s `300·n`.  Stamping with `v+1` against an
   invariant "every cell `≤ v`" removes it, and costs one store per
   candidate — which is already inside the `200·f` and `240·T` terms.
-/
...
theorem tpOff_spec {B : ℕ} {nN o t qo qt dg : String} {n : ℕ} {D : Orientation n}
  ...
  have hbody : Spec B
    ...
    · exact hfr.of_eq (by simp [hni, hna, hnd])
        ...
        (by simp [hnm.qo_dg, Ne.symm hnm.qo_dg])
        (by simp [hnm.qt_dg, hnm.qt_qo]) (by simp [Ne.symm hnm.qo_dg])
    · simp; omega
    · simp [hoffs]
    · intro k hk
      ...
      have harr : ((((((σ.setVar "tp.d" (outDegAt D i)).setArr qo i (outOff D i)).setArr
        ...
        simp [hnm.qo_dg, Ne.symm hnm.qo_dg]
      rw [harr]
...
theorem transposeIn_tpCom {B : ℕ} {nN o t qo qt dg : String}
  ...
  refine ⟨σ', _, hrun, ?_, hfr'.csr, ⟨_, outCsrAt_of_scat hfr' hqov' hns hst'⟩⟩
  simp only [tpK]
  omega
