...
parameters of `chainKB`, so those are hypotheses (`hKcovBot`,
`hKglue`), named and not buried — and **`SolveF7BridgeCover` discharges
both at the machine's own budgets**: `peelK a b c` on an edgeless arena
is exactly `(a+b)·A.N` (every cluster a singleton, every back-degree
zero), and the landed glue contract
(`SolveF7Adm.chainKB_frameStep_hKB`'s `hglue`) asks only `6 ≤ Kglue`.

**The cover column is admissibility-relative.**  `hKcov` — the edged
node's cover budget against the ledger's cover column — is asked for at
arenas satisfying an abstract `Adm` closed under the frame step's
children, not at every arena.  It has to be: the landed cover bound
`ProgCoverChargeDeg.exists_mcChargeMS_T_bucket_coverColumn` (clause 3)
holds at nodes whose arena embeds in the input, which
`SolveAugRoundIn.ardIsContained_of_chainAdm` supplies from `chainAdm`
and nothing supplies universally.  `b7_KsChargeBridge_all` recovers the
unrestricted form for a discharger who does have a universal bound.
...
constant.  The hypotheses are precisely the abstract slots of
`chainKB`: the channel bound the restrict column is proved at, the leaf
block's budget, and the cover and glue budgets — the cover one twice,
...
**The cover column is asked for only at admissible edged arenas.**  The
landed cover bound `ProgCoverChargeDeg.peelK_le_coverCFSel_total` carries
a weak-colouring hypothesis (`hdeg`), which is a fact about the arenas
the run actually reaches and *not* about every `Arena (S.pal i) n₀`; a
`hKcov` quantified over all arenas would therefore be a hypothesis the
machine's own cover budget cannot meet.  `Adm` is an arbitrary predicate
closed under the frame step's children (`hAdmChild`, verbatim
`SolveF7Adm.chainAdm_admChild`'s shape); `chainAdm` is the concrete
inhabited instance, and `Adm := fun _ _ => True` recovers the
unrestricted form.  The edgeless clause `hKcovBot` stays unrestricted —
below an edgeless node admissibility is *not* inherited
(`chainAdm_child` needs `¬ A.G = ⊥`), and it does not have to be: the
`⊥`-subtree bound needs no cover column at all. -/
theorem b7_chainKB_le
    (Adm : (j : ℕ) → Arena (S.pal j) n₀ → Prop)
    (hAdmChild : ∀ (j : ℕ) (A : Arena (S.pal j) n₀), Adm j A → ¬ A.G = ⊥ →
      ∀ u : Fin A.N, Adm (j + 1) (childArena S A ((ord A.N A.G).order) u))
    (hhbf : ∀ i, hbf i = 2 * S.R + 1)
    (hbotK : ∀ i : ℕ, i ≤ S.depth → ∀ A : Arena (S.pal i) n₀,
      botComK A.N (S.pal i) Kq (levelFml S i) ≤ M₀ * (A.N + 1))
    (hKcov : ∀ (i : ℕ) (A : Arena (S.pal i) n₀), i ≤ S.depth → Adm i A →
      ¬ A.G = ⊥ →
    ...
    (hKglue : ∀ (k i : ℕ) (A : Arena (S.pal i) n₀),
      Kglue k i A ≤ cglue * (A.N + 1)) :
    ∀ (k j : ℕ) (A : Arena (S.pal j) n₀), j + k ≤ S.depth → Adm j A →
      chainKB S ord Kq ℓp hbf Kcov Kglue k j A
  ...
  | succ k ih =>
    ...
    · -- the edged node: column by column
      ...
      have hbotc := hbotK j (by omega) A
      have hcov : Kcov j A ≤ ccov * (cov + A.N + 1) :=
        hKcov j A (by omega) hAdm hbot
      have hglue := hKglue k j A
      have hmax : max (botComK A.N (S.pal j) Kq (levelFml S j))
          (Kcov j A + ((((List.finRange A.N).map fun u =>
              centreK S A (ℓp j) (2 * S.R + 1) π u
...
The remaining hypotheses are the abstract slots of `chainKB` and of
`solveSpec_closed_scr` — nothing about the ledger or the chain is
assumed:

* `Adm` / `hAdmRoot` / `hAdmChild` — an admissibility predicate for the
  arenas the run reaches, closed under the frame step's children.  It is
  what the cover column's hypothesis is allowed to lean on;
  `SolveF7Adm.chainAdm` is the concrete inhabited instance and
  `headlineSetup_chainAdm_root` / `headlineSetup_chainAdm_admChild`
  discharge the two clauses hypothesis-free.  `Adm := fun _ _ => True`
  recovers the unrestricted form (`b7_KsChargeBridge_all`).
...
* `hKcov` / `hKcovBot` — the cover stage's budget, against the ledger's
  cover column at an admissible edged node and against the carrier at an
  edgeless one.  Two clauses because `frameChargeMS` has **no** cover
  column on the `A.G = ⊥` branch, while `frameElseK` pays `Kcov`
  unconditionally (`blockSpec_leaf_guard`'s `max`).  The edged clause is
  exactly the shape `ProgCoverChargeDeg.peelK_le_coverCFSel_total`
  delivers — including its weak-colouring side condition, which is why
  it is asked for at admissible arenas only.
* `hKglue` — the frame's glue slot, linear in the carrier.  The landed
  glue contract (`SolveF7Adm.chainKB_frameStep_hKB`'s `hglue`) asks only
  `6 ≤ Kglue k j A`, so the constant budget `6` meets both.
...
theorem b7_KsChargeBridge (C : GraphClass) (hC : NowhereDense C) (φ : FO 0)
  ...
  -- re-introduces those binders, and the hypothesis bundle stops applying).
  -- Every abbreviation below is `ℕ`-valued.
  have hchain := b7_chainKB_le (Headline.headlineSetup C hC φ) ord Kq ℓp hbf
    htabF covC Kcov Kglue M₀ ccov cglue Adm hAdmChild hhbf hbotK hKcov
    hKcovBot hKglue (Headline.headlineSetup C hC φ).depth 0
    (rootArena G (Impl.trivialColoring n)) (by omega) hAdmRoot
  ...
  set CTd := chargeTotal (driverChargeMS (Headline.headlineSetup C hC φ) ord ℓp
    htabF covC (Headline.headlineSetup C hC φ).depth 0
...
column asked for at every arena.  Kept because it is what a discharger
with a *universal* cover bound wants; the machine's own cover bound is
not universal (see `b7_KsChargeBridge`'s docstring), which is why the
admissibility-relative form above is the primary one. -/
theorem b7_KsChargeBridge_all (C : GraphClass) (hC : NowhereDense C) (φ : FO 0)
    (ord : CoverSpec.OrderingRoutine) {n : ℕ} (G : SimpleGraph (Fin n))
    (c w Kq : ℕ) (ℓp hbf : ℕ → ℕ)
    (htabF : (j : ℕ) →
      (A : Arena ((Headline.headlineSetup C hC φ).pal j) n) →
...
admissibility predicate `SolveF7Adm.chainAdm` — whose two clauses are the
hypothesis-free `headlineSetup_chainAdm_root` and
`headlineSetup_chainAdm_admChild`, and which `chainAdm_of_memTree` shows
holds at every arena the run tree visits — the canonical channel bound,
and the *non-zero* linear budgets `Kcov j A = A.N + 1`,
