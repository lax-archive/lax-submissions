...
structure ActiveCoreOut {n : ℕ} (B q r j : ℕ) (G : SimpleGraph (Fin n))
  ...
  cover : ∃ xp Xoff Xmem asg,
    ...
    CoverOutA G A₀ π centre r q xp Xoff Xmem asg ∧ xp < B
...
structure ActiveCoreAtOut {n : ℕ} (B q r j : ℕ) (G : SimpleGraph (Fin n))
    (A₀ : ℕ → ℕ) (π : Equiv.Perm (Fin n)) (centre : ℕ → ℕ)
    (σ : Env) : Prop where
  n_var : σ.vars "n" = n
  q_var : σ.vars "qn" = q
...
theorem activeCoreScratch_spec
    ...
    Spec B
      (ActiveInitPre B n ns nt q r j A₀ centre O T)
      (activeCoreScratchCom j r)
      (fun _ σ' => ActiveCoreOut B q r j G A₀ π centre σ') K := by
  intro σ hpre
  ...
  have hxpB : xp < B := lt_of_le_of_lt (raw_arena_le hraw) hnnB
  obtain ⟨σ₃, rsort, ⟨Xmem', Q, hxoff₃, hxmem₃, hasg₃, _hQ₃, hcover⟩,
      hfv, hfa, -, -⟩ :=
    ((radixCoverUniformCom_spec hraw (by omega) hnB hxpB
      (doubleWords_of_square (by omega) hnnB)).frame).run
  ...
  · refine
      { n_var := by rw [hfv "n" radixCoverUniform_preserves_n]; exact hn₂
        ...
        cover := ⟨xp, Xoff, Xmem', asg, by
          rw [hfv "xp" radixCoverUniform_preserves_xp]
          exact hxp₂, hxoff₃, hxmem₃, hasg₃, hcover, hxpB⟩ }

/-- The complete active core under the degree-priced arena bound.  Physical
arrays retain their `n²` allocation, but no runtime value is justified from
that allocation size. -/
theorem activeCoreScratchK_spec {Kball : ℕ}
    (hcentres : CentresBy n q A₀ π centre)
    (hcsr : Lax3Proofs.RamBfs.CsrGraph G ns O T)
  ...
  obtain ⟨σ₁, rinit, hinit⟩ :=
    (activeInit_spec hcentres hnB hqB hrB).run (σ := σ) hpre
  have hinitK : RawLoopStateK B ns nt q r Kball G A₀ π centre O T
      ((renEnv (activeCoverSwap j) σ₁).setVar "c" 0) := by
    obtain ⟨c₀, xp₀, Xoff₀, Xmem₀, asg₀, M₀, hstate₀⟩ := hinit
    refine ⟨c₀, xp₀, Xoff₀, Xmem₀, asg₀, M₀, hstate₀, ?_⟩
    have hc₀ : c₀ = 0 := by
      have hc := hstate₀.centre_var
      simpa using hc.symm
    have hxp₀ : xp₀ = 0 := by
      have hle := hstate₀.raw.ptr_le
      rw [hc₀] at hle
      simp at hle
      omega
    simp [hc₀, hxp₀]
...
theorem activeCoreAt_spec
  ...
  obtain ⟨xp, Xoff, Xmem, asg, hxp, hxoff, hxmem, hasg, hcover, hxpB⟩ := hout.cover
  ...
  refine
    { n_var := by simpa [renEnv] using hout.n_var
      ...
      cover := ⟨xp, Xoff, Xmem, asg, by simpa [renEnv] using hxp,
        ...
        by simpa [renEnv] using hasg, hcover, hxpB⟩ }

/-- Driver-facing degree-priced core, with no carrier-square value bound. -/
theorem activeCoreAtK_spec {Kball : ℕ}
    (hcentres : CentresBy n q A₀ π centre)
    (hcsr : Lax3Proofs.RamBfs.CsrGraph G ns O T)
    (hnB : n < B) (hnsB : ns < B) (hnt : ns ≤ nt)
    (hqB : q < B) (hrB : 2 * r + 1 < B)
...
#print axioms activeCoreScratchK_spec
#print axioms activeCoreAt_spec
#print axioms activeCoreAtK_spec
