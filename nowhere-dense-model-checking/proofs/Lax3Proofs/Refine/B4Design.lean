...
`n n mb n ns n`, where `20 + 37 + 65` collapses to the `122·n` of that
file's headline.
...
**Wave B4-walk-2m-3**: the carrier coefficient fell again, from `31` to
`20`, and the child's member count rose from `23` to `37` — the distance
fill's `11·n + 6` rewritten as `14·mm1 + 6` over the child's member list
(`RamDriver.distMemCom`). The carrier coefficient that is left is the
outside probe's alone, and at the narrowed charge
(`ScatterDeadTurn.deadAtomKX`) even that is capped. -/
...
coefficient, from `131` to `119`, and B4-walk-2m-3 to `122`, but by
changing the program text — not by accounting.) -/
theorem deadAtomK_cluster_floor {L : ℕ} (β : Lax3.DistFO.DistFO L 1) (m kq bw nb t : ℕ) :
    122 * m ≤ Lax3Proofs.Refine.ScatterDeadTurn.deadAtomK β m m kq m bw nb t := by
...
the constant term. `154·t + 122` is that slope: `44 + 110` per pick from
the ball, and `122` from the probe, the two member walks and the
distance fill at the cluster (wave E4c-c took the mask copy's `12` out
of it; B4-walk-2m-3 put the fill on the member list, which costs `3`
more at the cluster reading and removes the carrier). -/
theorem undersized_coeff_fails {L : ℕ} (β : Lax3.DistFO.DistFO L 1) (kq t ksc : ℕ)
  ...
  have hlow : (ksc + 1) * (ksc + 1) ≤ 154 * t * (ksc + 1) + 122 * (ksc + 1) := by
    have h1 : (ksc + 1) * (ksc + 1) ≤ (154 * t + 122) * (ksc + 1) :=
      Nat.mul_le_mul_right _ (by omega)
    have h2 : (154 * t + 122) * (ksc + 1) = 154 * t * (ksc + 1) + 122 * (ksc + 1) := by ring
    omega
