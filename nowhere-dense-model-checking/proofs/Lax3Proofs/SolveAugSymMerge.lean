...
Four sweeps, of which the middle three are the landed transpose:
...
1. **The merge needs no figure cell of its own.**  Its extent is the
   ...
   The carrier is the arena's `nN`, read once by `tpCom` and once by
   the merge's own scan, exactly as `TransposeIn` reads it (that pass's
   Finding 1).
...
theorem two_mul_arcCount_le_sq {N : ℕ} (D : Orientation N) :
    2 * arcCount D ≤ N * N := by
  classical
...
private theorem syRow_step {B : ℕ} {nN io it qo qt dg so st : String} {N : ℕ}
  ...
  have h2a : 2 * arcCount D ≤ N * N := two_mul_arcCount_le_sq D
...
theorem graphCsr_of_symRows {so st io it qo qt : String} {N : ℕ}
  ...
  have hInMem : ∀ w, ∀ hw : w < N, ∀ k, ∀ hk : k < off (w + 1) - off w,
    ...
    intro w hw k hk
    exact hin.sound ⟨w, hw⟩ (off w + k) (by simp only [Fin.val_mk]; omega)
  ...
  have hOutMem : ∀ w, ∀ hw : w < N, ∀ k, ∀ hk : k < outOff D (w + 1) - outOff D w,
    ...
    intro w hw k hk
    have := hout.sound ⟨w, hw⟩ (outOff D w + k) (by simp only [Fin.val_mk]; omega)
      (by simp only [Fin.val_mk]; omega) (hOutLt w hw k hk)
