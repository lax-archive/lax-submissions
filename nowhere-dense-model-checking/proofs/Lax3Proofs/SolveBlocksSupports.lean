...
def histBody (da pa ha : String) : Com :=
  .seq (.assign "sp.i"
...
def histCom (da pa ha : String) : Com :=
  .seq (.assign "sp.v" (.lit 0))
    (.while (.lt (.var "sp.v") (.var "sp.n")) (histBody da pa ha))
...
theorem noWrite_supportsCom (oa ta da pa ha : String) :
  ...
  simp [supportsCom, parentCom, spReset, spTurn, histCom, histBody,
...
theorem histBody_step :
  ...
  · -- the reached branch: materialize the walk
    ...
    have hbody : Run B (histBody da pa ha) σ σ' (histKb d) := by
