...
/-- **The machine leaf test is edgelessness**: the slot-count cell is
zero exactly when the arena's graph is empty — `GraphCsr`'s slot count
is the degree sum (`ns_eq_sum_degree`), and a graph of degree sum zero
is `⊥`. This is the guard the frame block's leaf branch tests (the
abstract dead-branch story stays in the run invariant). -/
theorem ArenaStW.ns_zero_iff_bot {nm : ArenaNames} {Λ n₀ ℓp hb : ℕ}
    {A : Impl.MArena Λ n₀ ℓp} {σ : Env} (h : ArenaStW nm hb A σ) :
    σ.vars nm.nS = 0 ↔ A.G = ⊥ := by
  classical
  have hns : σ.vars nm.nS = ∑ v : Fin A.N, A.G.degree v :=
    h.st.csr.ns_eq_sum_degree
  rw [hns, Finset.sum_eq_zero_iff]
  constructor
  · intro hdeg
    ext v w
    simp only [SimpleGraph.bot_adj, iff_false]
    intro hadj
    have hw : w ∈ A.G.neighborFinset v := by
      rwa [SimpleGraph.mem_neighborFinset]
    have hempty : A.G.neighborFinset v = ∅ :=
      Finset.card_eq_zero.mp (hdeg v (Finset.mem_univ v))
    rw [hempty] at hw
  ...
  · intro hbot v _
    ...
    rw [hbot]
    exact SimpleGraph.bot_adj .. |>.mp
  -- (the `..` supplies `w`, `v` to the `⊥`-adjacency reading)

/-- The slot count never exceeds `N²` — the bound that routes the leaf
guard's `< B` obligation through the word-size side condition. -/
theorem ArenaStW.ns_le_sq {nm : ArenaNames} {Λ n₀ ℓp hb : ℕ}
    {A : Impl.MArena Λ n₀ ℓp} {σ : Env} (h : ArenaStW nm hb A σ) :
    σ.vars nm.nS ≤ A.N * A.N := by
  classical
  have hns : σ.vars nm.nS = ∑ v : Fin A.N, A.G.degree v :=
    h.st.csr.ns_eq_sum_degree
  rw [hns]
  calc ∑ v : Fin A.N, A.G.degree v
      ≤ ∑ _v : Fin A.N, A.N := by
        refine Finset.sum_le_sum fun v _ => ?_
        have := A.G.degree_lt_card_verts v
