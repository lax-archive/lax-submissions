...
theorem reps_spec (colB : Fin n → Fin L → Bool) {col : Coloring n L}
  ...
  obtain ⟨-, hrow, hwoff⟩ : w ∈ Finset.univ ∧ (∀ c, colB w c = colB v c) ∧ ¬ ∃ i, m i = w := by
    have := F.min'_mem hne
    rwa [hF, Finset.mem_filter, and_assoc] at this
  ...
  · -- `w` is the first off-environment member of its row
    ...
    exact absurd (F.min'_le u huF) (not_le.mpr hu)
  · rw [← hcol, ← hcol, hrow c]
...
def botEval (colB : Fin n → Fin L → Bool) : {k : ℕ} → (Fin k → Fin n) →
  ...
  | _, m, .exL _ g φ => g.toList.any fun i => botEval colB (Fin.snoc m (m i)) φ
...
theorem botEval_eq_sat (colB : Fin n → Fin L → Bool) (col : Coloring n L)
  ...
  | not φ ih => intro m; simp [botEval, sat_not, ih]
  ...
  | exU φ ih =>
      ...
      constructor
      · rintro ⟨w, -, hw⟩
        exact sat_exU.mpr ⟨w, (ih (Fin.snoc m w)).mp hw⟩
      · intro hsat
  ...
  | exL r g φ ih =>
      intro m
      simp only [botEval, List.any_eq_true, Finset.mem_toList, sat_exL_bot]
      constructor
      · rintro ⟨i, hi, h⟩
