...
establishes it (`BlockPre` carries it), and it reaches the top stage by
`hscrLen0` — length preservation across the chain (`specArrsLength`)
converted back into the descriptor.

**That transport is not available to a descriptor that carries
content.** `SolveMachPrepSeam.rankScr_not_length_only` shows a `Scr`
implying the child-building pass's clean-scratch clause and admitting
`hscrLen0`'s length-only shape to be inconsistent. `hscrLen0` exists
only because `BlockPost` has no `Scr` conjunct, so nothing about the
descriptor survives `chainCom` on its own. `solveSpec_of_chain_scr`
below is this theorem with that gap closed at the source: the chain is
consumed at `BlockSpecScr`, whose postcondition contains `Scr 0`, and
`hscrLen0` disappears. Everything else — the hypotheses, the command,
the budget, the conclusion — is verbatim.
...
open Classical in
/-- **`SolveSpec`, closed by the chain — with no length-only
transport.** Verbatim `solveSpec_of_chain`: the same command, the same
budget, the same conclusion, and the same hypotheses save two —
`hscrLen0` is **gone**, and `hchain` is consumed at `BlockSpecScr`
(`BlockPost` plus the level's descriptor) instead of `BlockSpec`.

That is the whole exchange, and it is the honest one. `hscrLen0` was
the only bridge from `BlockPost` — which says nothing about `Scr` — to
the top stage's `Scr 0`, and it bridged by length preservation, which
`rankScr_not_length_only` refutes for any descriptor carrying the
child-building pass's clean-scratch clause. Here the root block hands
the descriptor over directly, so the top stage's precondition is met by
the chain's own postcondition and nothing is transported at all.

The obligation moves to the chain's two producers, where it can be
met: `botBlock_specScr` (this file — the leaf block writes no name the
descriptor reads) and `FrameStepScr` (the still-open residual 1, whose
loop invariant `CLInv` already carries `Scr j` at every centre). -/
theorem solveSpec_of_chain_scr
    (C : GraphClass) (hC : NowhereDense C) (φ : FO 0)
    (ord : CoverSpec.OrderingRoutine) {n : ℕ} (G : SimpleGraph (Fin n))
    (c w q : ℕ) (ext : List ℕ → String → ℕ)
    (ℓp : ℕ → ℕ)
    ...
    (hdep0 : (Headline.headlineSetup C hC φ).depth = 0 → G = ⊥)
    (hchain : ∀ x ∈ mcD n G c w,
      BlockSpecScr (mcB q x) (Headline.headlineSetup C hC φ) ord ℓp htabF hbf
        nmF Adm KB Scr (Headline.headlineSetup C hC φ).depth 0
        (chainCom frameBody botB (Headline.headlineSetup C hC φ).depth 0))
    (hload : ∀ x ∈ mcD n G c w,
      Spec (mcB q x) (MatIn (ext x) x) rootLoadCom
  ...
  obtain ⟨henc, hside⟩ := hx
  have h1B : 1 < mcB q x := one_lt_mcB (three_le_length henc) hq
  -- the chain, at the root arena, with the descriptor restored
  have hchainS := hchain x ⟨henc, hside⟩ (rootArena G (Impl.trivialColoring n))
    (Nat.zero_add _) hAdmRoot (fun h0 => hdep0 h0)
  -- the top scatter residual
  obtain ⟨Q, hscat, hav⟩ := htop x ⟨henc, hside⟩
  ...
  have htopS := topCom_spec (mcB q x) (Headline.headlineSetup C hC φ) ord G
    (Impl.trivialColoring n) av rfl h1B hscat hav
  -- chain ; top: the chain's own postcondition *is* the top stage's
  -- precondition — block postcondition and level-0 descriptor together
  have hct := Spec.seq hchainS htopS (fun _ _ _ hpost => hpost)
    (fun _ _ _ _ _ h => h)
  -- load ; (chain ; top)
  exact Spec.seq (hload x ⟨henc, hside⟩) hct
    (fun _ _ _ h => h) (fun _ _ _ _ _ h => h)
