...
/-- The cover state before block ordering is imposed.  This is the invariant
maintained by the touched-only breadth-first construction: every emitted
block is exact and repetition-free, but its queue order is intentionally not
claimed to be numeric order. -/
structure RawCoverInvA (G : SimpleGraph (Fin n)) (A₀ : ℕ → ℕ)
...
namespace RawCoverInvA

/-- Forgetting sortedness preserves every fact maintained by the raw
construction. -/
theorem of_sorted (hI : CoverInvA G A₀ π centre q r c xp Xoff Xmem asg M) :
    RawCoverInvA G A₀ π centre q r c xp Xoff Xmem asg M :=
  { pos_le := hI.pos_le
    zero := hI.zero
    mono := hI.mono
    ptr := hI.ptr
    ...
    asg_le := hI.asg_le
    asg_set := hI.asg_set
    asg_unset := hI.asg_unset }

/-- The block offsets are monotone throughout the processed raw prefix. -/
theorem mono' (hI : RawCoverInvA G A₀ π centre q r c xp Xoff Xmem asg M) {i j : ℕ}
    (hij : i ≤ j) (hj : j ≤ c) : Xoff i ≤ Xoff j := by
  induction j with
  | zero =>
      have : i = 0 := by omega
...
/-- The raw pass starts with the same empty arena and live assignment
sentinel as the sorted invariant. -/
theorem init (hA : ∀ u < n, M u = A₀ u) (hoff : Xoff 0 = 0)
    (hasg : ∀ w < n, A₀ w ≠ 0 → asg w = q) :
    RawCoverInvA G A₀ π centre q r 0 0 Xoff Xmem asg M where
  pos_le := Nat.zero_le _
  zero := hoff
  mono := fun _ hk => absurd hk (by omega)
  ptr := hoff
...
/-- One active centre advances the raw invariant.  Unlike `CoverInvA.step`,
this theorem asks only that the emitted queue segment be exact and
repetition-free; discovery order is unrestricted. -/
theorem step (hcentres : CentresBy n q A₀ π centre)
    ...
    (hasg : ∀ w < n, A₀ w ≠ 0 →
      asg' w = if asg w < q then asg w else if D w ≤ r then c else q)
    (hM : ∀ u < n, M' u = if u = centre c then 0 else M u) :
    RawCoverInvA G A₀ π centre q r (c + 1) xp' Xoff' Xmem' asg' M' := by
  have hv : centre c < n := hcentres.centre_lt c hc
  have hmg : masked G M =
      deleteVerts (masked G A₀) (pred π ⟨centre c, hv⟩) :=
    masked_step hcentres hc hI.mask
...
/-- At the end of the raw active prefix, all semantic cover clauses are
available.  `RawCoverOutA.sorted` is the sole remaining representation
step before recursive descent. -/
theorem out (hcentres : CentresBy n q A₀ π centre)
    (hI : RawCoverInvA G A₀ π centre q r q xp Xoff Xmem asg M) :
    RamCover.RawCoverOutA G A₀ π centre r q xp Xoff Xmem asg := by
  have hlt : ∀ w < n, A₀ w ≠ 0 → asg w < q := by
    intro w hw halive
    rcases lt_or_eq_of_le (hI.asg_le w hw halive) with h | heq
    · exact h
...
#print axioms RawCoverInvA.step
#print axioms RawCoverInvA.out
#print axioms RamCover.RawCoverOutA.sorted
