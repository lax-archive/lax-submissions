...
theorem botCom_specW {B Lc K n₀ ℓp : ℕ} {na fa ea xa : String} (hb : ℕ)
  ...
  -- reassemble the landed spec's name-list hypotheses
  have hnd' : ([nm.col, na, fa, ea, xa, nm.tab] : List String).Nodup := by
    simp only [List.nodup_cons, List.mem_cons, List.not_mem_nil, or_false,
      List.nodup_nil, and_true, not_or]
    exact ⟨⟨hca_na, hca_fa, hca_ea, hca_xa, hca_ta⟩,
      ⟨hna_fa, hna_ea, hna_xa, hna_ta⟩, ⟨hfa_ea, hfa_xa, hfa_ta⟩,
      ⟨hea_xa, hea_ta⟩, hxa_ta⟩
  have hoff' : nm.off ∉ ([na, fa, ea, xa, nm.tab] : List String) := by
    simp only [List.mem_cons, List.not_mem_nil, or_false, not_or]
    exact ⟨hoff_na, hoff_fa, hoff_ea, hoff_xa, hoff_ta⟩
  have htgt' : nm.tgt ∉ ([na, fa, ea, xa, nm.tab] : List String) := by
    simp only [List.mem_cons, List.not_mem_nil, or_false, not_or]
    exact ⟨htgt_na, htgt_fa, htgt_ea, htgt_xa, htgt_ta⟩
  have hup' : nm.up ∉ ([na, fa, ea, xa, nm.tab] : List String) := by
    simp only [List.mem_cons, List.not_mem_nil, or_false, not_or]
    exact ⟨hup_na, hup_fa, hup_ea, hup_xa, hup_ta⟩
  have hhist' : nm.hist ∉ ([na, fa, ea, xa, nm.tab] : List String) := by
    simp only [List.mem_cons, List.not_mem_nil, or_false, not_or]
    exact ⟨hhist_na, hhist_fa, hhist_ea, hhist_xa, hhist_ta⟩
  intro σ hσ
  ...
  have hws_na : ws na = none :=
    hws_scr na (Ne.symm hna_ta) (Ne.symm hoff_na) (Ne.symm htgt_na)
      (Ne.symm hca_na) (Ne.symm hup_na) (Ne.symm hhist_na)
  have hws_fa : ws fa = none :=
    hws_scr fa (Ne.symm hfa_ta) (Ne.symm hoff_fa) (Ne.symm htgt_fa)
      (Ne.symm hca_fa) (Ne.symm hup_fa) (Ne.symm hhist_fa)
  have hws_ea : ws ea = none :=
    hws_scr ea (Ne.symm hea_ta) (Ne.symm hoff_ea) (Ne.symm htgt_ea)
      (Ne.symm hca_ea) (Ne.symm hup_ea) (Ne.symm hhist_ea)
  have hws_xa : ws xa = none :=
    hws_scr xa (Ne.symm hxa_ta) (Ne.symm hoff_xa) (Ne.symm htgt_xa)
  ...
  have hnsv : σ'.vars nm.nS = ns := by
    rw [hns_def]
    exact hrun.frame_var nm.nS fun hm => hnS (wvars_botCom Fl hm)
  refine ⟨σ', hrun, ⟨?_, ?_⟩, ?_⟩
