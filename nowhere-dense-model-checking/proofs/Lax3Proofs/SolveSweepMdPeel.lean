...
private theorem getD_set_self (h : i < l.length) : (l.set i v).getD i 0 = v := by
  rw [List.getD_eq_getElem?_getD, List.getElem?_set_self h]
  rfl

/-- Reading past a store elsewhere. -/
private theorem getD_set_ne (h : k ≠ i) : (l.set i v).getD k 0 = l.getD k 0 := by
...
theorem bigStep_length_arrs {c : Com} {σ σ' : Env} {n : ℕ}
...
theorem spec_arrLengths {B : ℕ} {P : Env → Prop}
  ...
  obtain ⟨k, -, hbs⟩ := hr
  exact bigStep_length_arrs (hbs.bigStep) b
...
theorem heapPush_spec {n k : ℕ} {f : ℕ → ℕ} (kv : String)
  ...
  have h2 := (spec_arrLengths hcore).frame
  refine h2.post ?_
...
def mpRowBody (nNm aj dg hp hsv tv xv yv kv iv wv : String) : Com :=
            ...
            (.seq (.ite (.lt (.lit 0) (.get dg (.var vv)))
                (.seq (.assign iv (.get ao (.var vv)))
                  (.while (.lt (.var iv) (.get ao (.add (.var vv) (.lit 1))))
                    (mpRowBody nNm aj dg hp hsv tv xv yv kv iv wv)))
                .skip)
...
private theorem row_run
    ...
    ∃ σ' K' hn' fh', Run B
        (.while (.lt (.var iv) (.get ao (.add (.var vv) (.lit 1))))
          (mpRowBody nNm aj dg hp hsv tv xv yv kv iv wv)) σ₀ σ' K' ∧
  ...
  -- the loop invariant
  set bodyC : Com := mpRowBody nNm aj dg hp hsv tv xv yv kv iv wv with hbody
...
theorem mdPeelCore_spec (mt : String)
  ...
  · intro b hb
    ...
    refine hfa b ?_
    simp [mdPeelCom, initBody, roundCom, mpRowBody, chkCom, heapPushCom,
