...
towards a shared body of formalized mathematics. This document is itself a
Lax submission, made from the features it describes. A submission consists of
\emph{concepts}, \emph{proofs}, and optionally a \emph{paper}, which we
introduce in turn, using the prime numbers as a running example.
% lax end
\end{abstract}
...
readers only review the one thing Lean can \emph{not} check: whether the Lean
...
\href{https://laxarchive.org/lax-48/}{twin-width},
\href{https://laxarchive.org/lax-12/}{nowhere dense graph classes}, or
\href{https://laxarchive.org/lax-67/}{the word RAM}, Lax can help the
community agree on common standards.
...
Lax thereby tracks which lemmas were used to prove which other lemmas.
...
you may upload the same document to arXiv and to Lax. Using Radek
...
and mathlib evolves over time. Lax therefore declares one recommended
environment, the \emph{epoch}: a Lean toolchain together with a mathlib
commit, currently Lean~4.30.0 with mathlib at commit \texttt{c5ea003}.
Submissions may use any admitted environment (\texttt{lax doctor} lists
them) and are verified and archived either way, but only submissions in the
same environment can build on each other. The epoch advances rarely, about
once a year. When it does, an older submission is brought forward by a
revision, typically by whoever wants to build on it.
...
\end{verbatim}
and, inside an empty folder of a public git repository, run
\texttt{lax init}. This creates the layout the archive expects:
\begin{verbatim}
...
following result'' to your favorite coding agent is usually enough to fill
these folders with concepts and proofs. This may take several sessions,
