...
\author{\'Edouard Bonnet \and Jan Dreier \and Clemens Kuske}
\date{September 2026}
...
practices transfer to Lax: submissions may build upon each other, allowing common
standards and definitions to emerge by an organic selection process. The definition of a prime number is of course
...
\begin{proof}[Proof of Theorem~\ref{thm:euclid}]
  By Lemma~\ref{lem:primediv}, $n! + 1$ has a prime divisor $p$. If we had
  $p \leq n$, then $p$ would divide $n!$, and hence also $(n! + 1) - n! = 1$,
  which is impossible. So $p > n$.
\end{proof}
% lax end
...
A badge \laxproven{} next to a concept indicates that it has a complete
formal proof, while a badge \laxopen{} indicates that it does not. However, Lax
...
The proof above does this for its key step.

% lax begin Lax242665.PrimeDivisor
\begin{lemma}\label{lem:primediv}
  Every natural number greater than $1$ has a prime divisor.
\end{lemma}
% lax end

% lax begin Lax242665Proofs.PrimeDivisor.exists_prime_dvd
\begin{proof}
  Let $d$ be the least divisor of $n$ with $d > 1$. Every divisor of $d$
  divides $n$, so $d$ has no divisor strictly between $1$ and $d$; hence $d$
  is prime.
\end{proof}
% lax end
...
hyperedges are proofs between them. Here, Lemma~\ref{lem:primediv} is used to
derive Theorem~\ref{thm:euclid}. Clicking on a concept shows its proof network
and its open proof obligations.

A concept need not come with a proof at all.

% lax begin Lax242665.TwinPrimes
\begin{conjecture}\label{con:twin}
  For every natural number $n$ there is a prime $p > n$ such that $p + 2$ is
  prime as well.
\end{conjecture}
% lax end

Should a later submission prove Conjecture~\ref{con:twin}, it is marked as
proven \laxproven{} on the website, and so is every concept whose proof
depends on it. Providing a concept and proving it can thus be separate
contributions by separate people.
...
\begin{verbatim}
@misc{lax-242665,
  author       = {Bonnet, {\'E}douard and Dreier, Jan and Kuske, Clemens},
...
id, and the website points readers from the
...
version} of mathlib (currently \texttt{v4.33.0}). Submissions may use any
