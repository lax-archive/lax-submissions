...
black/red data and explicit update rules while submission Lax228581 phrases them
...
namespace Lax153141Proofs.Bridge
...
theorem homogeneous_iff {A B : Finset V} :
    Lax228581.TwinWidth.Homogeneous G A B ↔
...
theorem homogeneous_comm {A B : Finset V} :
    Lax228581.TwinWidth.Homogeneous G A B ↔ Lax228581.TwinWidth.Homogeneous G B A := by
...
theorem homogeneous_union_left [DecidableEq V] {A B Y : Finset V}
    ...
    Lax228581.TwinWidth.Homogeneous G (A ∪ B) Y ↔
      (Lax228581.TwinWidth.Homogeneous G A Y ∧ Lax228581.TwinWidth.Homogeneous G B Y ∧
        (CompleteBetween G A Y ↔ CompleteBetween G B Y)) := by
...
theorem homogeneous_union_right [DecidableEq V] {A B X : Finset V}
    ...
    Lax228581.TwinWidth.Homogeneous G X (A ∪ B) ↔
      (Lax228581.TwinWidth.Homogeneous G X A ∧ Lax228581.TwinWidth.Homogeneous G X B ∧
        (CompleteBetween G X A ↔ CompleteBetween G X B)) := by
...
theorem homogeneous_singleton (a b : V) :
    Lax228581.TwinWidth.Homogeneous G {a} {b} := by
...
def Faithful (G : SimpleGraph V) (T : TwinWidth.TrigraphState V) : Prop :=
  ∀ ⦃A B⦄, A ∈ T.bags → B ∈ T.bags → A ≠ B →
    ...
    (T.redAdj A B ↔ ¬ Lax228581.TwinWidth.Homogeneous G A B)
...
theorem contractedRed_iff [DecidableEq V]
    ...
    TwinWidth.SimpleGraph.contractedRed T A B X Y ↔
      ¬ Lax228581.TwinWidth.Homogeneous G X Y := by
...
theorem redDegree_eq_of_faithful [DecidableEq V]
    ...
    Lax228581.TwinWidth.redDegree G T.bags A = T.redDegree A := by
  classical
  unfold Lax228581.TwinWidth.redDegree TwinWidth.TrigraphState.redDegree
  rw [show {B | B ∈ T.bags ∧ B ≠ A ∧ ¬ Lax228581.TwinWidth.Homogeneous G A B} =
...
def submittedOfSource [Fintype V] [DecidableEq V] {d : ℕ}
    (S : TwinWidth.SimpleGraph.ContractionSequence G d) :
    Lax228581.TwinWidth.PartitionSequence G d where
...
theorem isPartitionFamily_singletonPartition [Fintype V] [DecidableEq V] :
    IsPartitionFamily (Lax228581.TwinWidth.singletonPartition V) := by
  refine ⟨?_, ?_, ?_⟩
  · intro A hA
...
theorem isPartitionFamily_partition [Fintype V] [DecidableEq V] {d : ℕ}
    (S : Lax228581.TwinWidth.PartitionSequence G d) :
...
def stateOf (G : SimpleGraph V) (P : Finset (Finset V))
  ...
  redAdj A B := A ≠ B ∧ ¬ Lax228581.TwinWidth.Homogeneous G A B
...
def clampedPartition [Fintype V] [DecidableEq V] {d : ℕ}
    (S : Lax228581.TwinWidth.PartitionSequence G d) (i : ℕ) :
    Finset (Finset V) :=
...
def sourceOfSubmitted [Fintype V] [DecidableEq V] {d : ℕ}
  ...
  starts := by
    have h0 : clampedPartition S 0 = Lax228581.TwinWidth.singletonPartition V := by
      rw [clampedPartition_of_le S (Nat.zero_le _), S.starts]
    refine ⟨h0, ?_, ?_⟩
    · intro A B hA hB
      have hA' : A ∈ Lax228581.TwinWidth.singletonPartition V := h0 ▸ hA
      have hB' : B ∈ Lax228581.TwinWidth.singletonPartition V := h0 ▸ hB
  ...
  redDegree_le i hi A hA := by
    ...
    rw [← hred]
    show Lax228581.TwinWidth.redDegree G (clampedPartition S i) A ≤ d
...
theorem submitted_hasTwinWidthAtMost_iff [Fintype V] [DecidableEq V] {d : ℕ} :
    Lax228581.TwinWidth.HasTwinWidthAtMost G d ↔
...
theorem submittedDivision_disjoint {n k : ℕ}
    (D : Lax153141.MixedMinorNumber.Division n k) ⦃i j : Fin k⦄ (hij : i ≠ j) :
...
theorem submittedDivision_convex {n k : ℕ}
    (D : Lax153141.MixedMinorNumber.Division n k) (i : Fin k) ⦃a b c : Fin n⦄
...
def sourceDivisionOfSubmitted {n k : ℕ}
    (D : Lax153141.MixedMinorNumber.Division n k) : TwinWidth.Division n k where
  part := D.part
...
def submittedDivisionOfSource {n k : ℕ} (D : TwinWidth.Division n k) :
    Lax153141.MixedMinorNumber.Division n k where
  part := D.part
  part_nonempty := D.part_nonempty
  part_cover := D.part_cover
  part_ordered := D.part_ordered
...
theorem cellVertical_iff (hMP : ∀ r c, (M r c = true) ↔ P r c)
    (R : Lax153141.MixedMinorNumber.Division n k)
    (C : Lax153141.MixedMinorNumber.Division m k) (i j : Fin k) :
    TwinWidth.Matrix.CellVertical M (sourceDivisionOfSubmitted R)
        (sourceDivisionOfSubmitted C) i j ↔
      Lax153141.MixedMinorNumber.cellVertical P R C i j := by
  constructor
...
theorem cellHorizontal_iff (hMP : ∀ r c, (M r c = true) ↔ P r c)
    ...
    TwinWidth.Matrix.CellHorizontal M (sourceDivisionOfSubmitted R)
        (sourceDivisionOfSubmitted C) i j ↔
      Lax153141.MixedMinorNumber.cellHorizontal P R C i j := by
  constructor
...
theorem cellMixed_iff (hMP : ∀ r c, (M r c = true) ↔ P r c)
    ...
    TwinWidth.Matrix.CellMixed M (sourceDivisionOfSubmitted R)
        (sourceDivisionOfSubmitted C) i j ↔
      Lax153141.MixedMinorNumber.cellMixed P R C i j :=
...
theorem hasMixedMinor_iff (hMP : ∀ r c, (M r c = true) ↔ P r c)
    (hk : k ≠ 0) :
    Lax153141.MixedMinorNumber.HasMixedMinor P k ↔
...
theorem hasMixedMinor_le_min
    (h : Lax153141.MixedMinorNumber.HasMixedMinor P k) : k ≤ min n m := by
  obtain ⟨R, C, -⟩ := h
...
theorem matrixMixedNumber_eq (hMP : ∀ r c, (M r c = true) ↔ P r c) :
    Lax153141.MixedMinorNumber.matrixMixedNumber P =
      TwinWidth.Matrix.matrixMixedNumber M := by
  classical
  apply le_antisymm
  · rcases Set.eq_empty_or_nonempty
        {k | Lax153141.MixedMinorNumber.HasMixedMinor P k} with he | hne
    · show sSup {k | Lax153141.MixedMinorNumber.HasMixedMinor P k} ≤ _
      rw [he, csSup_empty]
      exact bot_le
    · show sSup {k | Lax153141.MixedMinorNumber.HasMixedMinor P k} ≤ _
      apply csSup_le hne
  ...
  · have hsrc := TwinWidth.Matrix.hasMixedMinor_matrixMixedNumber M
    ...
    · rw [hg0]
      exact Nat.zero_le _
    · have hsub : Lax153141.MixedMinorNumber.HasMixedMinor P
          (TwinWidth.Matrix.matrixMixedNumber M) :=
        (hasMixedMinor_iff hMP hg0).mpr hsrc
      show _ ≤ sSup {k | Lax153141.MixedMinorNumber.HasMixedMinor P k}
...
theorem orderedAdjacency_bridge {n : ℕ} (G : SimpleGraph V) (e : Fin n ≃ V) :
    ∀ i j, (TwinWidth.Matrix.orderedAdjacency G ⟨e⟩ i j = true) ↔
      Lax153141.MixedMinorNumber.orderedAdjacency G e i j := by
  intro i j
  unfold TwinWidth.Matrix.orderedAdjacency Lax153141.MixedMinorNumber.orderedAdjacency
  simp
...
theorem submitted_orderedMixedNumber_eq {n : ℕ}
    ...
    Lax153141.MixedMinorNumber.matrixMixedNumber
        (Lax153141.MixedMinorNumber.orderedAdjacency G e) =
...
theorem submitted_mixedMinorNumber_eq [Fintype V] [DecidableEq V]
    (G : SimpleGraph V) :
    Lax153141.MixedMinorNumber.mixedMinorNumber G =
      TwinWidth.SimpleGraph.mixedMinorNumber G := by
  show sInf (Set.range fun e : Fin (Fintype.card V) ≃ V =>
      Lax153141.MixedMinorNumber.matrixMixedNumber
        (Lax153141.MixedMinorNumber.orderedAdjacency G e)) = _
  apply le_antisymm
  · obtain ⟨σ, hσ⟩ :=
