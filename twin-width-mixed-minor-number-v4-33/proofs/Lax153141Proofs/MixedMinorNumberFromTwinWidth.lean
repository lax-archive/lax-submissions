...
source twin-width; `Lax153141Proofs.Bridge` proves both submitted parameters
pointwise equal to their source counterparts, so the bound transports
verbatim.
-/

namespace Lax153141Proofs.MixedMinorNumberFromTwinWidth

noncomputable section

/--
---
conclusion: Lax153141.MixedMinorNumberFromTwinWidth.exists_mixedMinorNumber_bound_of_twinWidth
---
The submitted mixed minor number is bounded by a numerical function of
twin-width (the parameter of submission Lax228581); the witness is the linear
function `d ↦ 2 * (d + 3) + 2`.
...
`Lax153141Proofs.Bridge` moves the bound onto the submitted parameters.
...
theorem exists_mixedMinorNumber_bound_of_twinWidth :
    ∃ g : ℕ → ℕ, ∀ {V : Type} [Fintype V] [DecidableEq V]
      ...
      Lax153141.MixedMinorNumber.mixedMinorNumber G ≤
        g (Lax228581.TwinWidth.twinWidth G) := by
...
end Lax153141Proofs.MixedMinorNumberFromTwinWidth
