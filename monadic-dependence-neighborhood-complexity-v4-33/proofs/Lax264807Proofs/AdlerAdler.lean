...
`Lax264807.AdlerAdler.monadicallyDependent_of_nowhereDense`.
...
and `Lax264807Proofs.QuasiWideness` only reshapes the conclusion from `Set`
...
theorem monadicallyDependent_of_nowhereDense
  ...
  obtain ⟨N, s, huqw⟩ := Lax264807Proofs.QuasiWideness.uqw_of_nowhereDense C h
    (3 ^ (q + 2))
  ...
  have hps : Lax264807.GraphTransductions.structureClass allGraphs (d + 2 ^ d)
...
end Lax264807Proofs.AdlerAdler
