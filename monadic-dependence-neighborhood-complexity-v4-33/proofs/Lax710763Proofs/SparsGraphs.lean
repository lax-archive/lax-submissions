...
open FirstOrder Lax710763.Transductions Lax710763.GraphClasses
open Lax199508.GraphClasses Lax199508.NeighborhoodComplexity
open Lax710763.MonadicDependence
...
def sparsGraphs (C : GraphClass) (k : ℕ) : GraphClass := fun m H =>
  TransductionCalculus.transductionImage (sparsTransduction k)
    (Lax710763.GraphTransductions.structureClass C) m H.structure ∧
...
theorem transduces_sparsGraphs (C : GraphClass) (k : ℕ) :
    Lax710763.GraphTransductions.Transduces C (sparsGraphs C k) :=
  TransductionCalculus.Transduces.mono_target
    (TransductionCalculus.transduces_transductionImage
      (sparsTransduction k) (Lax710763.GraphTransductions.structureClass C))
    fun _ _ hS => by
      obtain ⟨H, hH, rfl⟩ := hS
      exact hH.1
...
theorem hasAlmostLinearNC_sparsGraphs {C : GraphClass}
  ...
  Lax710763Proofs.Corollary6.hasAlmostLinearNC_of_weaklySparse_of_monadicallyDependent
