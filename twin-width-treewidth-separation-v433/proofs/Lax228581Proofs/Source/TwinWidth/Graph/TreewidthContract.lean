...
@[simp] theorem oneBagTreeDecomposition_width
  ...
  classical
  rw [oneBagTreeDecomposition, TreeDecomposition.width]
  change (Finset.univ.sup (fun _ : Unit => Fintype.card V) - 1 = Fintype.card V - 1)
  rw [Finset.sup_const (s := (Finset.univ : Finset Unit)) Finset.univ_nonempty]
