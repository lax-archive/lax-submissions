...
theorem eq_root_of_level_zero {branch depth : ℕ}
  ...
  rcases u with ⟨⟨n, hn⟩, path⟩
  cases n with
  | zero =>
    cases h
    have hfst : (⟨0, hn⟩ : Fin (depth + 1)) = (⟨0, by omega⟩ : Fin (depth + 1)) := by
      ext
      rfl
    cases hfst
    congr
  ...
  symm :=
    (Std.Symm.mk (fun a b h => by
      ...
      | inr u =>
        cases b with
        | inl x => exact h
        | inr v => exact (FullTreeNode.graph (bonnetDepresBranch k) (bonnetDepresDepth k)).symm.symm u v h))
  loopless := by
    constructor
    intro a h
    cases a with
