...
theorem ContractionSequence.hasManyChildrenInPart_of_nonSingleton_tree_bag
  ...
  hasManyChildrenInPart_of_nonSingleton_tree_bag_of_rootChildrenSingleton
    ...
    hd (Lax228581Proofs.TwinWidth.SimpleGraph.TrigraphState.isBagPartition (S.state i))
    hroot (Lax228581Proofs.TwinWidth.SimpleGraph.ContractionSequence.partitionRedDegreeAtMost_state S hi)
...
theorem ContractionSequence.hasManyPChildren_of_hasManyChildrenInPart_of_nonpreleaf
  ...
  Lax228581Proofs.TwinWidth.SimpleGraph.BonnetDepres.hasManyPChildren_of_hasManyChildrenInPart_of_nonpreleaf
...
theorem ContractionSequence.qProperty_of_hasManyChildrenInPart
  ...
  Lax228581Proofs.TwinWidth.SimpleGraph.BonnetDepres.qProperty_of_hasManyChildrenInPart
    (k := k) (d := d) (P := (S.state i).bags) (u := u) (hlevel := hlevel)
    hd (Lax228581Proofs.TwinWidth.SimpleGraph.TrigraphState.isBagPartition (S.state i))
    hroot (Lax228581Proofs.TwinWidth.SimpleGraph.ContractionSequence.partitionRedDegreeAtMost_state S hi)
    hP
...
theorem ContractionSequence.hasManyChildrenInPart_step
  ...
  hasManyChildrenInPart_mono_of_isBagContraction
    (Lax228581Proofs.TwinWidth.SimpleGraph.IsContractionStep.isBagContraction
...
theorem ContractionSequence.hasManyChildrenInPart_mono
  ...
  | @step n hij ih =>
      ...
      exact
        Lax228581Proofs.TwinWidth.SimpleGraph.BonnetDepres.ContractionSequence.hasManyChildrenInPart_step
...
theorem ContractionSequence.qProperty_mono
  ...
  | @step n hij ih =>
      ...
      exact
        Lax228581Proofs.TwinWidth.SimpleGraph.BonnetDepres.ContractionSequence.qProperty_step
          S (Nat.lt_of_succ_le hj) hprev
...
theorem ContractionSequence.exists_hasManyChildrenInPart_before_of_not_tree_singleton
  ...
  have hpartP : IsBagPartition (S.state p).bags :=
    Lax228581Proofs.TwinWidth.SimpleGraph.TrigraphState.isBagPartition (S.state p)
  have hpartQ : IsBagPartition (S.state (p + 1)).bags :=
    Lax228581Proofs.TwinWidth.SimpleGraph.TrigraphState.isBagPartition (S.state (p + 1))
  have hredQ :
      PartitionRedDegreeAtMost (bonnetDepresGraph k) (S.state (p + 1)).bags d :=
    Lax228581Proofs.TwinWidth.SimpleGraph.ContractionSequence.partitionRedDegreeAtMost_state
      S (by
        have hfirst_le :=
          ContractionSequence.firstRootChildrenNonSingletonIndex_le_stepCount S
        omega)
...
theorem ContractionSequence.exists_rootChild_hasManyChildrenInPart_before_first
  ...
  have hpartPrev : IsBagPartition (S.state i).bags :=
    Lax228581Proofs.TwinWidth.SimpleGraph.TrigraphState.isBagPartition (S.state i)
  have hpartFirst :
      IsBagPartition (S.state (ContractionSequence.firstRootChildrenNonSingletonIndex S)).bags :=
    Lax228581Proofs.TwinWidth.SimpleGraph.TrigraphState.isBagPartition
  ...
  · let u := rootChildWithNeighborhood k f₀
    have hsingleP :
      ...
      have hA' : rootChildBag k f₀ ∈ (S.state i).bags := by
        simpa [hAroot] using hA
      simpa [u, rootChildBag, i] using hA'
  ...
  · let u := rootChildWithNeighborhood k f₀
    have hsingleP :
        ({Sum.inr u} : Finset (BonnetDepresVertex k)) ∈ (S.state i).bags := by
      have hB' : rootChildBag k f₀ ∈ (S.state i).bags := by
        simpa [hBroot] using hB
      simpa [u, rootChildBag, i] using hB'
...
theorem ContractionSequence.treeVertex_singleton_of_not_qProperty_before_firstRootChildQIndex
  ...
  have hPpred :
    ...
    Lax228581Proofs.TwinWidth.SimpleGraph.BonnetDepres.ContractionSequence.hasManyChildrenInPart_mono
  ...
  have hQpred :
      QProperty (S.state (i - 1)).bags u :=
    Lax228581Proofs.TwinWidth.SimpleGraph.BonnetDepres.ContractionSequence.qProperty_of_hasManyChildrenInPart
