...
namespace Lax228581Proofs.Main
...
def submittedTreeDecompositionOfSource
    ...
    Lax228581.Treewidth.TreeDecomposition G where
  Node := D.Node
...
theorem treewidth_le_of_source
    ...
    (D : TwinWidth.SimpleGraph.TreeDecomposition G)
    (hwidth : D.width ≤ w) :
    Lax228581.Treewidth.treewidth G ≤ w := by
...
theorem hasTwinWidthAtMost_mono [Fintype V] [DecidableEq V] {d e : ℕ}
    (h : Lax228581.TwinWidth.HasTwinWidthAtMost G d) (hde : d ≤ e) :
    Lax228581.TwinWidth.HasTwinWidthAtMost G e := by
  obtain ⟨S⟩ := h
  exact ⟨{ S with
...
theorem lt_twinWidth_of_not_hasTwinWidthAtMost
    ...
    (hnot : ¬ Lax228581.TwinWidth.HasTwinWidthAtMost G d)
    (hex : ∃ e, Lax228581.TwinWidth.HasTwinWidthAtMost G e) :
    d < Lax228581.TwinWidth.twinWidth G := by
  have hne : {e | Lax228581.TwinWidth.HasTwinWidthAtMost G e}.Nonempty := hex
  have hmem : Lax228581.TwinWidth.HasTwinWidthAtMost G
      (Lax228581.TwinWidth.twinWidth G) := Nat.sInf_mem hne
  by_contra hle
...
theorem homogeneous_image {V V' : Type} {G : SimpleGraph V}
    {G' : SimpleGraph V'} [DecidableEq V'] (e : G ≃g G') (A B : Finset V) :
    Lax228581.TwinWidth.Homogeneous G' (A.image e.toEquiv) (B.image e.toEquiv) ↔
      Lax228581.TwinWidth.Homogeneous G A B := by
...
theorem image_singletonPartition {V V' : Type} [Fintype V] [DecidableEq V]
    ...
    (Lax228581.TwinWidth.singletonPartition V).image (Finset.image φ) =
      Lax228581.TwinWidth.singletonPartition V' := by
  ext A
  simp only [Lax228581.TwinWidth.singletonPartition, Finset.mem_image]
  constructor
  · rintro ⟨B, ⟨v, -, rfl⟩, rfl⟩
...
theorem redDegree_image {V V' : Type} [DecidableEq V] [DecidableEq V']
    ...
    Lax228581.TwinWidth.redDegree G' (P.image (Finset.image e.toEquiv))
        (A.image e.toEquiv) =
      Lax228581.TwinWidth.redDegree G P A := by
  unfold Lax228581.TwinWidth.redDegree
  ...
  rw [show {B | B ∈ P.image (Finset.image e.toEquiv) ∧
        B ≠ A.image e.toEquiv ∧
        ¬ Lax228581.TwinWidth.Homogeneous G' (A.image e.toEquiv) B} =
      Finset.image e.toEquiv ''
        {B | B ∈ P ∧ B ≠ A ∧ ¬ Lax228581.TwinWidth.Homogeneous G A B} from ?_]
...
theorem exists_treewidth_le_and_two_pow_lt_twinWidth (k : ℕ) :
    ∃ n : ℕ, ∃ G : SimpleGraph (Fin n),
      Lax228581.Treewidth.treewidth G ≤ 2 * k + 4 ∧
        2 ^ k < Lax228581.TwinWidth.twinWidth G := by
  classical
  ...
  · have hnot : ¬ Lax228581.TwinWidth.HasTwinWidthAtMost G' (2 ^ k) := by
      rintro ⟨S'⟩
      ...
      exact
        TwinWidth.SimpleGraph.BonnetDepres.bonnetDepres_not_hasTwinWidthAtMost_two_pow
          k hsource
    have hex : ∃ e, Lax228581.TwinWidth.HasTwinWidthAtMost G' e := by
