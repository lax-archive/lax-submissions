...
def _root_.Lax808846Proofs.Imp.Com.NoWrite : Com → Prop
  | .skip => True
  | .assign _ _ => True
  | .store _ _ _ => True
  | .read _ => True
...
under `Lax808846Proofs`, which the archive's namespace rule rejects. Asking
here is enough: every later `simp [Com.NoWrite]` finds the equations
among its imports and creates nothing. -/
theorem _root_.Lax808846Proofs.Imp.Com.noWrite_skip : Com.skip.NoWrite := by
  simp [Com.NoWrite]

/-- A command that contains no `write` leaves the output tape alone. -/
theorem _root_.Lax808846Proofs.Imp.BigStep.out_eq {c : Com} {σ σ' : Env} {k : ℕ}
