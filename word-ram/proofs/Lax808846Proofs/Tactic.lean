...
namespace Lax808846Proofs.Reasoning
...
partial def evalE (cfg : Cfg) (mv : MVarId) (σ e : Lean.Expr) :
  ...
  | (``Lax808846Proofs.Imp.Expr.lit, #[n]) =>
      let h ← mkSide cfg mv (← natLt n cfg.B)
      return (n, mkAppN (mkConst ``RunStep.eval_lit) #[cfg.B, n, σ, h])
  | (``Lax808846Proofs.Imp.Expr.var, #[x]) =>
      let v := mkApp2 (mkConst ``Lax808846Proofs.Imp.Env.vars) σ x
      let h ← mkSide cfg mv (← natLt v cfg.B)
      return (v, mkAppN (mkConst ``RunStep.eval_var) #[cfg.B, σ, x, h])
  | (``Lax808846Proofs.Imp.Expr.get, #[a, i]) =>
      let (k, hi) ← evalE cfg mv σ i
      let arr := mkApp2 (mkConst ``Lax808846Proofs.Imp.Env.arrs) σ a
  ...
  | (``Lax808846Proofs.Imp.Expr.bin, #[op, e₁, e₂]) =>
      let (m, h₁) ← evalE cfg mv σ e₁
      let (n, h₂) ← evalE cfg mv σ e₂
      let op ← withReducible <| whnf op
      let (rule, v) ← match op.getAppFnArgs with
        | (``Lax808846Proofs.Imp.Bop.add, _) => pure (``RunStep.eval_add, ← mkAppM ``HAdd.hAdd #[m, n])
        | (``Lax808846Proofs.Imp.Bop.sub, _) => pure (``RunStep.eval_sub, ← mkAppM ``HSub.hSub #[m, n])
        | (``Lax808846Proofs.Imp.Bop.mul, _) => pure (``RunStep.eval_mul, ← mkAppM ``HMul.hMul #[m, n])
        | (``Lax808846Proofs.Imp.Bop.div, _) => pure (``RunStep.eval_div, ← mkAppM ``HDiv.hDiv #[m, n])
        | (``Lax808846Proofs.Imp.Bop.and, _) => pure (``RunStep.eval_and, ← mkAppM ``Nat.land #[m, n])
        | (``Lax808846Proofs.Imp.Bop.or, _) => pure (``RunStep.eval_or, ← mkAppM ``Nat.lor #[m, n])
        | (``Lax808846Proofs.Imp.Bop.xor, _) => pure (``RunStep.eval_xor, ← mkAppM ``Nat.xor #[m, n])
        | (``Lax808846Proofs.Imp.Bop.shiftl, _) =>
            pure (``RunStep.eval_shiftl, ← mkAppM ``HMul.hMul #[m, ← mkAppM ``HPow.hPow #[mkNatLit 2, n]])
        | (``Lax808846Proofs.Imp.Bop.shiftr, _) =>
...
private partial def flattenSeq (c : Lean.Expr) : MetaM (Array Lean.Expr) := do
  ...
  | (``Lax808846Proofs.Imp.Com.seq, #[c₁, c₂]) => return #[c₁] ++ (← flattenSeq c₂)
  | _ => return #[c]

private def mkSeq (a b : Lean.Expr) : Lean.Expr :=
  mkApp2 (mkConst ``Lax808846Proofs.Imp.Com.seq) a b
...
partial def exec (cfg : Cfg) (mv : MVarId) (c σ : Lean.Expr) (specs : Array Lean.Expr)
  ...
  | (``Lax808846Proofs.Imp.Com.store, #[a, i, e]) =>
      ...
      let (σ', K) ← runParts ty
      kont mv σ' K h specs
  | (``Lax808846Proofs.Imp.Com.read, #[x]) =>
      let inp := mkApp (mkConst ``Lax808846Proofs.Imp.Env.inp) σ
      ...
      let (σ', K) ← runParts ty
      kont mv σ' K h specs
  | (``Lax808846Proofs.Imp.Com.write, #[e]) =>
      let (v, he) ← evalE cfg mv σ e
      let val := mkAppN (mkConst ``RunStep.write) #[cfg.B, σ, e, v, he]
      let (h, ty, mv) ← mkHave mv `hrun val
      let (σ', K) ← runParts ty
      kont mv σ' K h specs
  | (``Lax808846Proofs.Imp.Com.seq, #[c₁, c₂]) =>
