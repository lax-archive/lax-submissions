...
0. **One namespace per module inside `Lib`.** Everything a module owns
   ...
   exist and a downstream `open Lax808846Proofs.Reasoning.Lib` stays
...
def layout : Lax808846Proofs.Compile.Layout := ⟨["i", "r"], ["mk"], 2⟩

/-- The machine program. -/
def prog : Lax808846.Ram.Program :=
  Lax808846Proofs.Compile.compileProgram layout (demoWatched "mk" "i" "r")

/-- The layout covers the block, so the compilation is the one the
simulation theorem is about and not an accident. -/
theorem demoWatched_ok : Lax808846Proofs.Compile.Com.Ok layout (demoWatched "mk" "i" "r") := by
  simp [demoWatched, mark, unmark, test, layout, Lax808846Proofs.Compile.Com.Ok,
