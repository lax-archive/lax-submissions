...
def layout : Lax808846Proofs.Compile.Layout := ⟨["hd", "tl", "x", "y", "r"], ["q"], 3⟩

/-- The machine program. -/
def prog : Lax808846.Ram.Program :=
  Lax808846Proofs.Compile.compileProgram layout (demoWatched "q" "hd" "tl" "x" "y" "r")
...
theorem demoWatched_ok :
    Lax808846Proofs.Compile.Com.Ok layout (demoWatched "q" "hd" "tl" "x" "y" "r") := by
  simp [demoWatched, push, front, advance, drain, layout, Lax808846Proofs.Compile.Com.Ok,
...
def demoRun : Option (List ℕ × ℕ) := runOut 16 2000 prog (Lax808846.Ram.initState []) 0

#guard demoRun = some ([5, 5, 7], 112)
