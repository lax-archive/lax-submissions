...
namespace Lax199508Proofs.AdmissibilityBound

open Lax199508.Admissibility Lax199508.ShallowTopologicalMinors
open Lax199508Proofs.MinorBridge Lax199508Proofs.OrderBridge

/--
---
conclusion: Lax199508.AdmissibilityBound.adm_le_of_hasTopologicalDensityAtMost
---
...
`Lax199508Proofs.AdmByDensity.adm_le_of_topGrad_bound`.
-/
theorem adm_le_of_hasTopologicalDensityAtMost {n : ℕ} (G : SimpleGraph (Fin n))
  ...
  have hd : ∀ {W : Type} [DecidableEq W] [Fintype W] (H : SimpleGraph W)
      [DecidableRel H.Adj],
      Lax199508Proofs.TopologicalMinors.IsShallowTopologicalMinor H G (r + 1 - 1) →
    ...
    exact hcount
  obtain ⟨ord, hadm⟩ :=
    Lax199508Proofs.AdmByDensity.adm_le_of_topGrad_bound G (r + 1) d hd
  letI := ord
  refine Nat.sInf_le ⟨rankPerm ord, fun v j hj => ?_⟩
  obtain ⟨F⟩ := hj
  have h1 : j + 1 ≤ Lax199508Proofs.OrderedParameters.admVertex G (r + 1) v :=
    le_admVertex_of_admFamily F
  have h2 : Lax199508Proofs.OrderedParameters.admVertex G (r + 1) v ≤
      Lax199508Proofs.OrderedParameters.adm G (r + 1) :=
    Finset.le_sup (f := fun x => Lax199508Proofs.OrderedParameters.admVertex G (r + 1) x)
      (Finset.mem_univ v)
  omega

end Lax199508Proofs.AdmissibilityBound
