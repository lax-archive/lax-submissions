...
namespace Lax199508Proofs.OrderBridge

open Lax199508.ColoringNumbers Lax199508.Admissibility
...
theorem wcol_le_catalog {m : ℕ} (ord : LinearOrder (Fin m))
    ...
    letI := ord
    Lax199508.ColoringNumbers.wcol H r ≤ Lax199508Proofs.OrderedParameters.wcol H r := by
  letI := ord
  refine Nat.sInf_le (⟨rankPerm ord, fun v => ?_⟩ :
    ∃ π : Equiv.Perm (Fin m), ∀ v, (wreach H π r v).ncard ≤
      Lax199508Proofs.OrderedParameters.wcol H r)
  calc (wreach H (rankPerm ord) r v).ncard
      ≤ (WReach H r v).ncard := Set.ncard_le_ncard (wreach_subset_WReach H r ord v)
    _ ≤ Lax199508Proofs.OrderedParameters.wcol H r := by
        unfold Lax199508Proofs.OrderedParameters.wcol
...
theorem scol_le_catalog {m : ℕ} (ord : LinearOrder (Fin m))
    (H : SimpleGraph (Fin m)) (r : ℕ) :
    letI := ord
    Lax199508.ColoringNumbers.scol H r ≤ Lax199508Proofs.OrderedParameters.scol H r := by
  letI := ord
  refine Nat.sInf_le (⟨rankPerm ord, fun v => ?_⟩ :
    ∃ π : Equiv.Perm (Fin m), ∀ v, (sreach H π r v).ncard ≤
      Lax199508Proofs.OrderedParameters.scol H r)
  calc (sreach H (rankPerm ord) r v).ncard
      ≤ (SReach H r v).ncard := Set.ncard_le_ncard (sreach_subset_SReach H r ord v)
    _ ≤ Lax199508Proofs.OrderedParameters.scol H r := by
...
theorem catalog_scol_le {m : ℕ} (π : Equiv.Perm (Fin m))
    ...
    letI := orderOfPerm π
    Lax199508Proofs.OrderedParameters.scol H r ≤ k := by
  letI := orderOfPerm π
  unfold Lax199508Proofs.OrderedParameters.scol
...
theorem catalog_adm_le {π : Equiv.Perm (Fin m)} (G : SimpleGraph (Fin m)) (r k : ℕ)
    ...
    letI := orderOfPerm π
    Lax199508Proofs.OrderedParameters.adm G r ≤ k := by
  classical
  letI := orderOfPerm π
  unfold Lax199508Proofs.OrderedParameters.adm
  refine Finset.sup_le fun v _ => ?_
  have h1 : 1 ≤ k := by
...
theorem adm_le_adm_succ (G : SimpleGraph (Fin m)) (r : ℕ) :
    Lax199508.Admissibility.adm G r ≤ Lax199508.Admissibility.adm G (r + 1) := by
  refine Nat.sInf_le ?_
  obtain ⟨π, hπ⟩ : HasAdmAtMost G (r + 1) (Lax199508.Admissibility.adm G (r + 1)) :=
...
end Lax199508Proofs.OrderBridge
