...
| plan | what |
|------|------|
| `port-v4.33/` | the v4.33.0 port of Jan's submissions (2026-09-14): four drafts in place (word-ram, ram-linear-time, refinement-tower, nowhere-dense-model-checking, plus lax-introduction) and two successors of registered records (lax-264807 ⊃ lax-5, lax-768004 ⊃ lax-48), every package green locally, no statement changed; `README.md` is the per-site ledger with the drift classes, `FINISH.md` the one-off prompt that lands the branch and resubmits the chain bottom-up, `WORKER-BRIEF.md` the porting packet. Open until the chain is resubmitted. |
