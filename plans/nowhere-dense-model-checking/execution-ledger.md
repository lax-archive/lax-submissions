...
| F6c10a | `CentrePrepAll` + `CentreReadAll` | ready | — | — | the two straight-line segments of `SolveStep.lean` §3: prep = cluster-row read (`ClusterCsr.read_row`) → restrict → BFS 2R → supports → profilesMS at `preG` → isolate → region load, delivering verbatim `BlockPre` at `childArena` with `htabF (j+1)`/`Scr (j+1)`; read = scatters on the isolated child + `bcExpr` readback of rows `{v | centre v = u}` matching `frameEval`'s recursive clause (`levelFml_rank`), `CLInv u → CLInv (u+1)`; all stage lifts landed; `ProfNames.Ok` gaps off `lv` |
| F6c10b | `CoverAllIn` + `RootCsrLoadAll` | ready | — | — | (1) `CoverAllIn` (`SolveGlueStep` §4): GKS peeling sweep per admissible level arena — **needs a peel-masked BFS** (mask the peeled-off vertices; `bfsCom_specW` is full-graph — either a masked variant or restrict-then-BFS per E11's forced route), identities `sweepCluster_eq_cluster`/`sweepCtr_eq_centre`, budget `sweepCharge`; (2) `RootCsrLoadAll` (`SolveGlueLoad` §1): dedup mark-array CSR compaction O(n+m) from `MatIn`, slot count into level-0 `nS` |
