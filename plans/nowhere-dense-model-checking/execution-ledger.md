...
| F6c12-5c-i | `TransCsrIn` | done (part) | w26 | see log | `transCsrIn_trCom` + `transCsrAt_decides` (the both-directions `O(1)` test the `pick` filter needs) + `transCsrAt_row`, `trOff_owner`, `trOuter_scan`, `trRow_lt_sq`, `transCsrAt_slots_le`. **Budget verified**: `trK_le : trK n (arcCount D) (transPairCount D) ≤ n*(27 + 23d + 30d²) + 13` under `D.InDegLE d` — the `transPairCount` shape, `O(n·d²)`. Full semantic review of the `Spec` still owed. | **Split off from 5c and dispatched w26** — also not blocked (the peel runs after). The transitive enumeration: for `v`, for `w ∈ inN v`, for `u ∈ inN w`, emit `(u,v)`; regroup + dedupe. **Directed and not symmetric**, and the consumer's `pick` filter tests `TransLink D u v` *and* `TransLink D v u`, so the region must answer both in `O(1)` (CSR + transpose). Budget target pre-verified: `transPairCount D = Σ_v Σ_{w∈inN v} \|inN w\|`, priced by `levelCharge`. Note `TransLink` permits `u = v` as stated but `Orientation.asymm` makes that vacuous — to be *proved* vacuous, not assumed. |
