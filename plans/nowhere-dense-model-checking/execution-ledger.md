...
| F6c11 | the four machine passes | wip (a landed part) | w21/w22 | 6e9b52e | **w21 landed**: `machChild_eq_ofArena` (the assembly IS `Impl.ofArena childArena` under the profiles witness), `ChildLoadParts` named at `ChildLoad`'s exact Spec, `childLoadAll_of_parts`/`centrePrepAll_of_parts` verbatim under `hhtab`; the `htabF` canonicity kit (`ballTable_eq_ballDist`, `descendCol_eq_descendTab`, `preG_eq_comap_childUp` — `htabF (j+1)` well-defined per child arena; per-round sources must be child-recoverable, carried by `hhtab`); readback bit bridges (`rowAsn_local_bit`, `rowAsn_scatter_bit` on the ISOLATED child, `headlineSetup_choice` by `rfl`). Remaining machine passes: `ChildLoadPartsAll` (stage composition + glue), `ReadRowsAll` (programs+frames only now), `CovOrderIn`/`CovSweepIn` (w22 running) |
