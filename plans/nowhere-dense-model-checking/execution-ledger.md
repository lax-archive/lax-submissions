...
| F6c8 | FrameStep glue + the last three residuals | ready | — | — | (1) `FrameStepAll`: the stage lifts are ALL landed (`SolveChainBot`/`SolveChainRestrict`/`SolveFrameStages`) — remaining is the per-centre glue (cluster-row copy into `la`, batch from child HistArr + `up`, per-centre count region, readback via `bcExpr` per schedule row), the centre loop, leaf branch via `blockSpec_leaf_guard`; instantiate supports at **2R** (`hdhb : d+1 ≤ hb`, `hb = 2R+1`); discharge `ProfNames.Ok`'s missing up/hist freshness hypotheses off `lv`; (2) `CoverStageSpec`: per-centre `bfsCom` on the peeled graph + `sweepCluster_eq_cluster`/`sweepCtr_eq_centre`, budget `sweepCharge`; (3) `RootLoadSpec` (`MatIn` → level-0 `BlockPre` CSR copy) + `TopScatterAll` (from `topCom_spec`); (4) pin `KB := frameK`, prove `KsChargeBridge` against `solveSpec_closed`'s named budget |
| F7 | discharge the endorsed axiom | waiting | — | — | needs F6c8; then: `Adm` at the run tree (`mkSetup_memLeaf_eq_bot`), instantiate `SolveSpec` via `solveSpec_closed`, `q`/`c` per `hspan`, `temps ≥` boolean depth, `T x := L.const·mcK`, reconcile `Ks` with `exists_mcChargeMS_T`, ∃-close with the `conclusion:` header |
