...
| F6c11 | the four machine passes | ready | — | — | (1) `ChildLoadAll` (`SolveSegPrep` §1): restrict→bfs 2R→supports→profilesMS at `preG`→isolate chain + glue, delivering the windowed contract at `Impl.ofArena childArena` with `htabF (j+1)`, level-`j` names untouched; (2) `ReadRowsAll` (`SolveSegRead` §5): write exactly the centre-`u` rows at `RowEval`'s bits via per-atom scatters + `bcExprA` readback; (3) `CovOrderIn`: machine min-degree peel rank array (F7 repins `ord` — keep parametric where possible); (4) `CovSweepIn`: GKS frontier-queue sweep over a deletable adjacency region (`Lib.Queue` exists; the deletable-adjacency invariant is the new machinery), budget `sweepCharge`'s two summands. Then KB pin + `KsChargeBridge`, then F7 |
