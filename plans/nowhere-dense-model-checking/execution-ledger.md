...
| F6c9a | `CentreStepAll` — the per-centre body | ready | — | — | one straight-line pass re-establishing `CLInv u → CLInv (u+1)` (SolveGlueLoop §3): window the centre's regions, run restrict → bfs(2R) → supports → profilesMS → isolate → recurse-window → scatter → readback per schedule row; ALL stage lifts landed (`SolveChainBot/Restrict`, `SolveFrameStages`); supports at `hdhb : 2R+1 ≤ hb`; `ProfNames.Ok`'s up/hist freshness off `lv`; budget `KC` per centre |
| F6c9b | TopScatter fix + `CoverAllIn` + `RootCsrLoadAll` | ready | — | — | (1) the statement fix: add length-only top-scratch pre to `TopScatterSpec` (`SolveChain.lean`), thread through `solveSpec_of_chain`, `TopScatterAll`, `solveSpec_closed` (`SolveFrameBridge.lean`) — narrow explicit ownership of exactly those declarations; then discharge `TopScatterAll` via `topCom_spec` + scatter machinery; (2) `CoverAllIn`: per-centre `bfsCom` on the peeled graph + `sweepCluster_eq_cluster`/`sweepCtr_eq_centre`, budget `sweepCharge`; (3) `RootCsrLoadAll`: the deduplicating CSR pass (mark-array compaction) |
| F7 | discharge the endorsed axiom | waiting | — | — | needs F6c9a+b, then KB pin + `KsChargeBridge`; then: `Adm` at the run tree (`mkSetup_memLeaf_eq_bot`), instantiate `SolveSpec` via `solveSpec_closed`, `q`/`c` per `hspan`, `temps ≥` boolean depth, `T x := L.const·mcK`, reconcile `Ks` with `exists_mcChargeMS_T`, ∃-close with the `conclusion:` header |
