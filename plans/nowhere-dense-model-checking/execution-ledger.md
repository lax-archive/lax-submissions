...
| F6c6 | the frame + `ℓ+1` chain: close `SolveSpec` | done (part) | w14 | 59d19c2 | windowed contract landed (`bigStepB_padA`, `specWindow`, `ArenaStW`/`TableBitsW`); botCom/restrictCom lifted; level chain proved (`chainCom_blockSpec`); leaf guard machine-tested (`ns_zero_iff_bot` ⟺ edgeless); **`solveSpec_of_chain`**: `SolveSpec` for `matCom ; rootLoadCom ; chainCom ; topCom scatCom` conditional on FOUR residuals — root `BlockSpec` (= FrameStep), root load, `TopScatterSpec`, `Adm` — budget `Ks`, `KsChargeBridge` named; supervisor decision: supports runs at radius **2R** (§5 l.17), `ProgFrame.supportsC`'s `S.R` slot is the stale figure |
| F6c7 | discharge the four chain residuals | ready | — | — | (1) `FrameStep` = root `BlockSpec`: lift isolate/bfs/supports/profiles/scatter through `specWindow` (mechanical per the botCom/restrictCom templates), per-centre glue (cluster-row copy into `la`, batch from child HistArr + up, per-centre count region, readback via `bcExpr` per schedule row), the centre loop, leaf branch via `blockSpec_leaf_guard`; supports at **2R** not `S.R`; (2) `CoverStageSpec`: per-centre `bfsCom` on the peeled graph + `sweepCluster_eq_cluster`/`sweepCtr_eq_centre`, budget `sweepCharge`; (3) root load (`MatIn` → level-0 `BlockPre` CSR copy) + `TopScatterSpec`; (4) pin `KB := frameK`, prove `KsChargeBridge` |
| F7 | discharge the endorsed axiom | waiting | — | — | needs F6c7; then: `Adm` at the run tree (`mkSetup_memLeaf_eq_bot`), instantiate `SolveSpec` via `solveSpec_of_chain`, `q`/`c` per `hspan`, `temps ≥` boolean depth, `T x := L.const·mcK`, reconcile `Ks` with `exists_mcChargeMS_T`, ∃-close with the `conclusion:` header |
